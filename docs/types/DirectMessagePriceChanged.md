...
| :--- | :--- | :---: | :--- |
| areDirectMessagesEnabled | `boolean` | Yes | True, if direct messages are enabled for the channel chat; False otherwise |
| directMessageStarCount | `number` | No | Optional. The new number of Telegram Stars that must be paid by users for each direct message sent to the channel. Does not apply to users who have been exempted by administrators. Defaults to 0. |

## Fluent Methods

The `DirectMessagePriceChanged` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendMessage({ chatId: "Response" });
});
```
...
| :--- | :--- | :--- |
| `messageId` | `this.areDirectMessagesEnabled?.id` | Message identifier in the chat specified in from_chat_id |
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendContact({
  chatId: 123,
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendContact({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendPoll({
  chatId: 123,
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendChecklist({
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendDice({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editChatInviteLink({
  chatId: 123,
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editForumTopic(
  123,
  123,
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendGift({ giftId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editStory({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `messageId` | `this.areDirectMessagesEnabled?.id` | Required if inline_message_id is not specified. Identifier of the message to edit. |
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `messageId` | `this.areDirectMessagesEnabled?.id` | Unique identifier for the target message |
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const directmessagepricechanged = new DirectMessagePriceChanged(rawData, bot);
await directmessagepricechanged.sendGame({
  chatId: 123,
...
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  // Auto-fills parameters from the directmessagepricechanged instance
  await directmessagepricechanged.sendGame({ chatId: "Response" });
});
```
...
You can listen for DirectMessagePriceChanged events using:

```typescript
// Type-specific handler
bot.onDirectMessagePriceChanged(async (directmessagepricechanged: DirectMessagePriceChanged) => {
  console.log('Received:', directmessagepricechanged);
  // Use fluent methods
  await directmessagepricechanged.sendMessage(...);
});

// Generic handler
bot.on('directmessagepricechanged', async (data: DirectMessagePriceChanged) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#DirectMessagePriceChanged).
