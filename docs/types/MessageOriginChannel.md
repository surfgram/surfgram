...
| authorSignature | `string` | No | Optional. Signature of the original post author |

## Fluent Methods

The `MessageOriginChannel` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendMessage({
  text: "example text",
...
```typescript
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendMessage({ text: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.forwardMessage({
  messageThreadId: 123,
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.forwardMessage({ messageEffectId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.forwardMessages({
  messageIds: [123],
  messageThreadId: 123,
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendAudio({ audio: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendDocument({ document: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendVideo({ video: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendVoice({ voice: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendVideoNote({ videoNote: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendVenue({ title: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendContact({
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendContact({ phoneNumber: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendPoll({
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendPoll({ question: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendDice({
  businessConnectionId: "example text",
  messageThreadId: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendChatAction(
  "example text",
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editChatInviteLink({
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editChatInviteLink({ inviteLink: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editChatSubscriptionInviteLink(
  "example text",
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendChatJoinRequestWebApp(
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editForumTopic(
  123,
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editGeneralForumTopic(
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendGift({ giftId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageText({
  businessConnectionId: "example text",
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageCaption({
  businessConnectionId: "example text",
  inlineMessageId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editMessageReplyMarkup({
  businessConnectionId: "example text",
  inlineMessageId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editEphemeralMessageText({
  receiverUserId: 123,
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editEphemeralMessageText({ text: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editEphemeralMessageMedia({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editEphemeralMessageCaption({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.editEphemeralMessageCaption({ caption: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendInvoice({
  title: "example text",
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendInvoice({ title: "Response" });
});
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const messageoriginchannel = new MessageOriginChannel(rawData, bot);
await messageoriginchannel.sendGame({
...
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  // Auto-fills parameters from the messageoriginchannel instance
  await messageoriginchannel.sendGame({ gameShortName: "Response" });
});
```
...
You can listen for MessageOriginChannel events using:

```typescript
// Type-specific handler
bot.onMessageOriginChannel(async (messageoriginchannel: MessageOriginChannel) => {
  console.log('Received:', messageoriginchannel);
  // Use fluent methods
  await messageoriginchannel.sendMessage(...);
});

// Generic handler
bot.on('messageoriginchannel', async (data: MessageOriginChannel) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageOriginChannel).
