...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getManagedBotToken();
```
...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getManagedBotAccessSettings();
```
...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getUserGifts({
...
bot.onChatMemberMember(async (chatmembermember: ChatMemberMember) => {
  // Auto-fills parameters from the chatmembermember instance
  await chatmembermember.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatmembermember = new ChatMemberMember(rawData, bot);
await chatmembermember.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatMemberMember(async (chatmembermember: ChatMemberMember) => {
  // Auto-fills parameters from the chatmembermember instance
  await chatmembermember.getChatGifts({ offset: "Response" });
});
```
