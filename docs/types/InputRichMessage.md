...
| :--- | :--- | :---: | :--- |
| blocks | `InputRichBlock[]` | No | Optional. Content of the rich message to send described as a list of blocks |
| html | `string` | No | Optional. Content of the rich message to send described using HTML formatting. See rich message formatting options for more details. Use media field to specify the media used in the message. |
| markdown | `string` | No | Optional. Content of the rich message to send described using Markdown formatting. See rich message formatting options for more details. Use media field to specify the media used in the message. |
| media | `InputRichMessageMedia[]` | No | Optional. List of media that are specified in the markdown or html fields using tg://photo?id=, tg://video?id=, and tg://audio?id= links |
| isRtl | `boolean` | No | Optional. Pass True if the rich message must be shown right-to-left |
| skipEntityDetection | `boolean` | No | Optional. Pass True to skip automatic detection of entities \(e.g., URLs, email addresses, username mentions, hashtags, cashtags, bot commands, or phone numbers\) in the text |

## Fluent Methods

The `InputRichMessage` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendContact({
  chatId: 123,
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendPoll({
  chatId: 123,
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendChecklist({
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editChatInviteLink({
  chatId: 123,
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editForumTopic(
  123,
  123,
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessage = new InputRichMessage(rawData, bot);
await inputrichmessage.sendGame({
  chatId: 123,
...
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  // Auto-fills parameters from the inputrichmessage instance
  await inputrichmessage.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputRichMessage events using:

```typescript
// Type-specific handler
bot.onInputRichMessage(async (inputrichmessage: InputRichMessage) => {
  console.log('Received:', inputrichmessage);
  // Use fluent methods
  await inputrichmessage.sendMessage(...);
});

// Generic handler
bot.on('inputrichmessage', async (data: InputRichMessage) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputRichMessage).
