...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getManagedBotToken(
  123,
);
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUserGifts({
  userId: 123,
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getChatGifts({ offset: "Response" });
});
```
