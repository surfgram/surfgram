...
| gift | `Gift`   |    No    | Optional. The gift sent to the chat by the bot |

## Fluent Methods

The `TransactionPartnerChat` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getUpdates({});
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getWebhookInfo({
  url: 'example text',
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getWebhookInfo({ url: 'Response' });
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMe({
  text: 'example text',
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMe({ text: 'Response' });
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUserProfilePhotos(123, 123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getUserProfilePhotos();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getFile('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getFile();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChat();
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChatAdministrators();
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChatMemberCount();
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChatMember(123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getChatMember();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getForumTopicIconStickers('example text', 123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getForumTopicIconStickers();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getUserChatBoosts(123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getUserChatBoosts();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getBusinessConnection('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getBusinessConnection();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyCommands({} as any, 'example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyCommands();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyName('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyName();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyDescription('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyDescription();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyShortDescription('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyShortDescription();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getChatMenuButton();
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyDefaultAdministratorRights(true);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyDefaultAdministratorRights();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getAvailableGifts({
  userId: 123,
  payForUpgrade: true,
});
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getAvailableGifts({ text: 'Response' });
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getBusinessAccountStarBalance('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getBusinessAccountStarBalance();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getStickerSet('example text');
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getStickerSet();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getCustomEmojiStickers(['example text']);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getCustomEmojiStickers();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getMyStarBalance(123, 123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getMyStarBalance();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getStarTransactions(123, 123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getStarTransactions();
});
```
...
```typescript
const transactionpartnerchat = new TransactionPartnerChat(rawData, bot);
await transactionpartnerchat.getGameHighScores(123, 123);
```
...
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  // Auto-fills parameters from the transactionpartnerchat instance
  await transactionpartnerchat.getGameHighScores();
});
```
...
You can listen for TransactionPartnerChat events using:

```typescript
// Type-specific handler
bot.onTransactionPartnerChat(async (transactionpartnerchat: TransactionPartnerChat) => {
  console.log('Received:', transactionpartnerchat);
  // Use fluent methods
  await transactionpartnerchat.getUpdates(...);
});

// Generic handler
bot.on('transactionpartnerchat', async (data: TransactionPartnerChat) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#TransactionPartnerChat).
