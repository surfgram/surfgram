...
| thumbnail | `PhotoSize` | No | Optional. Document thumbnail as defined by the sender |
...
The `Document` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const document = new Document(rawData, bot);
await document.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendMessage({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendPhoto({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendAudio({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendDocument({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendVideo({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendAnimation({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendVoice({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendPaidMedia({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendLocation({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendVenue({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendContact({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendPoll({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendDice({ chatId: "Response" });
});
```
...
```typescript
const document = new Document(rawData, bot);
await document.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const document = new Document(rawData, bot);
await document.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendGift({ giftId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const document = new Document(rawData, bot);
await document.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const document = new Document(rawData, bot);
await document.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendInvoice({ chatId: "Response" });
});
```
...
bot.onDocument(async (document: Document) => {
  // Auto-fills parameters from the document instance
  await document.sendGame({ chatId: "Response" });
});
```
...
You can listen for Document events using:
...
// Generic handler
bot.on('document', async (data: Document) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#Document).
