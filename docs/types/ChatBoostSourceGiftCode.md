...
The `ChatBoostSourceGiftCode` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getUpdates({});
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getWebhookInfo({
  url: 'example text',
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getWebhookInfo({ url: 'Response' });
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMe({
  text: 'example text',
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMe({ text: 'Response' });
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUserProfilePhotos(123, 123);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getUserProfilePhotos();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getFile('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getFile();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChat();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChatAdministrators();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChatMemberCount();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChatMember();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getForumTopicIconStickers();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUserChatBoosts();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getBusinessConnection('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getBusinessConnection();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyCommands({} as any, 'example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyCommands();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyName('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyName();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyDescription('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyDescription();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyShortDescription('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyShortDescription();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChatMenuButton();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyDefaultAdministratorRights(true);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getAvailableGifts({
  payForUpgrade: true,
  text: 'example text',
});
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getAvailableGifts({ text: 'Response' });
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getBusinessAccountStarBalance('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getStickerSet('example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getStickerSet();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getCustomEmojiStickers(['example text']);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getCustomEmojiStickers();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getMyStarBalance(123, 123);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getMyStarBalance();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getStarTransactions(123, 123);
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getStarTransactions();
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getGameHighScores(123, 'example text');
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getGameHighScores();
});
```
...
You can listen for ChatBoostSourceGiftCode events using:

```typescript
// Type-specific handler
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  console.log('Received:', chatboostsourcegiftcode);
  // Use fluent methods
  await chatboostsourcegiftcode.getUpdates(...);
});

// Generic handler
bot.on('chatboostsourcegiftcode', async (data: ChatBoostSourceGiftCode) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatBoostSourceGiftCode).
