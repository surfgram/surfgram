...
| :--- | :--- | :---: | :--- |
| source | `string` | Yes | Source of the boost, always “gift\_code” |
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getManagedBotToken();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getManagedBotAccessSettings();
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getUserGifts({
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostsourcegiftcode = new ChatBoostSourceGiftCode(rawData, bot);
await chatboostsourcegiftcode.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostSourceGiftCode(async (chatboostsourcegiftcode: ChatBoostSourceGiftCode) => {
  // Auto-fills parameters from the chatboostsourcegiftcode instance
  await chatboostsourcegiftcode.getChatGifts({ offset: "Response" });
});
```
