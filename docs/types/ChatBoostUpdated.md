...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getManagedBotToken(
  123,
);
```
...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getUserGifts({
  userId: 123,
...
bot.onChatBoostUpdated(async (chatboostupdated: ChatBoostUpdated) => {
  // Auto-fills parameters from the chatboostupdated instance
  await chatboostupdated.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostupdated = new ChatBoostUpdated(rawData, bot);
await chatboostupdated.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostUpdated(async (chatboostupdated: ChatBoostUpdated) => {
  // Auto-fills parameters from the chatboostupdated instance
  await chatboostupdated.getChatGifts({ offset: "Response" });
});
```
