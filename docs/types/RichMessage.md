...
| :--- | :--- | :---: | :--- |
| blocks | `RichBlock[]` | Yes | Content of the message |
| isRtl | `boolean` | No | Optional. True, if the rich message must be shown right-to-left |

## Fluent Methods

The `RichMessage` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.forwardMessages({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendDocument({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendAnimation({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendLocation({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendVenue({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendContact({ chatId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendChecklist({
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendDice({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editChatInviteLink({
  chatId: 123,
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editForumTopic(
  123,
  123,
);
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editGeneralForumTopic(
  123,
  "example text",
);
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendGift({ giftId: "Response" });
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const richmessage = new RichMessage(rawData, bot);
await richmessage.editUserStarSubscription(
  123,
  "example text",
);
```
...
bot.onRichMessage(async (richmessage: RichMessage) => {
  // Auto-fills parameters from the richmessage instance
  await richmessage.sendGame({ chatId: "Response" });
});
```
...
You can listen for RichMessage events using:

```typescript
// Type-specific handler
bot.onRichMessage(async (richmessage: RichMessage) => {
  console.log('Received:', richmessage);
  // Use fluent methods
  await richmessage.sendMessage(...);
});

// Generic handler
bot.on('richmessage', async (data: RichMessage) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#RichMessage).
