...
| :--- | :--- | :---: | :--- |
| users | `User[]` | Yes | New members that were invited to the video chat |

## Fluent Methods

The `VideoChatParticipantsInvited` class has the following fluent methods that automatically inject contextual parameters:
...
await videochatparticipantsinvited.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getWebhookInfo({
  url: "example text",
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getWebhookInfo({ url: "Response" });
});
```
...
| :--- | :--- | :--- |
| `receiverUserId` | `this.users?.id` | For outgoing ephemeral messages, unique identifier of the user who will receive the message; for group and supergroup chats only. It is not guaranteed that the user will receive the message, especially if they are offline. See ephemeral message sending for more details. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMe({
  text: "example text",
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getMe({ text: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendMessage({
  text: "example text",
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendMessage({ text: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendAudio({ audio: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendDocument({ document: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendVideo({ video: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendVoice({ voice: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendVideoNote({ videoNote: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. If the chat is a channel, all Telegram Star proceeds from this media will be credited to the chat's balance. Otherwise, they will be credited to the bot's balance. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendVenue({ title: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendContact({
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendContact({ phoneNumber: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. Polls can't be sent to channel direct messages chats. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendPoll({
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendPoll({ question: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this?.id` | Unique identifier for the target chat or username of the target bot in the format @username |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendDice({
  businessConnectionId: "example text",
  messageThreadId: 123,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendMessageDraft({ text: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this?.id` | Unique identifier for the target chat or username of the target bot or supergroup in the format @username. Channel chats and channel direct messages chats aren't supported. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendChatAction(
  "example text",
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | Unique identifier of the target user |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getFile(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChat();
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChatAdministrators(
  true,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChatMemberCount();
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChatMember();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | Unique identifier for the target user |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getUserChatBoosts();
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getBusinessConnection(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | User identifier of the managed bot whose token will be returned |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getManagedBotToken();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | User identifier of the managed bot whose access settings will be returned |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getManagedBotAccessSettings();
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyName(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyDescription(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyShortDescription(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChatMenuButton();
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyDefaultAdministratorRights(
  true,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | Required if chat_id is not specified. Unique identifier of the target user who will receive the gift. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getAvailableGifts({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendGift({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendGift({ giftId: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | Unique identifier of the user |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getUserGifts({
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getStickerSet(
  "example text",
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendInvoice({
  title: "example text",
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendInvoice({ title: "Response" });
});
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getStarTransactions(
  123,
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this?.id` | Unique identifier for the target chat or username of the target bot in the format @username. Games can't be sent to channel direct messages chats and channel chats. |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.sendGame({
...
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  // Auto-fills parameters from the videochatparticipantsinvited instance
  await videochatparticipantsinvited.sendGame({ gameShortName: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.users?.id` | Target user id |
...
```typescript
const videochatparticipantsinvited = new VideoChatParticipantsInvited(rawData, bot);
await videochatparticipantsinvited.getGameHighScores(
  123,
  "example text",
);
```
...
You can listen for VideoChatParticipantsInvited events using:

```typescript
// Type-specific handler
bot.onVideoChatParticipantsInvited(async (videochatparticipantsinvited: VideoChatParticipantsInvited) => {
  console.log('Received:', videochatparticipantsinvited);
  // Use fluent methods
  await videochatparticipantsinvited.getUpdates(...);
});

// Generic handler
bot.on('videochatparticipantsinvited', async (data: VideoChatParticipantsInvited) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#VideoChatParticipantsInvited).
