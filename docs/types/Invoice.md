...
| startParameter | `string` |   Yes    | Unique bot deep-linking parameter that can be used to generate this invoice                                                                                                                                                                                                                     |
...
The `Invoice` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const invoice = new Invoice(rawData, bot);
...
```typescript
bot.onInvoice(async (invoice: Invoice) => {
  // Auto-fills parameters from the invoice instance
  await invoice.sendInvoice({ chatId: 'Response' });
});
```
...
```typescript
const invoice = new Invoice(rawData, bot);
await invoice.createInvoiceLink({
  title: 'example text',
...
bot.onInvoice(async (invoice: Invoice) => {
  // Auto-fills parameters from the invoice instance
  await invoice.createInvoiceLink({ title: 'Response' });
});
```
...
You can listen for Invoice events using:
...
// Generic handler
bot.on('invoice', async (data: Invoice) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#Invoice).
