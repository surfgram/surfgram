...
The `ChatInviteLink` class has the following fluent methods that automatically inject contextual parameters:
...
await chatinvitelink.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
| :--- | :--- | :--- |
| `receiverUserId` | `this.creator?.id` | For outgoing ephemeral messages, unique identifier of the user who will receive the message; for group and supergroup chats only. It is not guaranteed that the user will receive the message, especially if they are offline. See ephemeral message sending for more details. |
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.exportChatInviteLink();
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getChatAdministrators(
  true,
);
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getChatMemberCount();
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getChatMember();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.creator?.id` | Unique identifier for the target user |
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getUserChatBoosts();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.creator?.id` | User identifier of the managed bot whose token will be returned |
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getManagedBotToken();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.creator?.id` | User identifier of the managed bot whose access settings will be returned |
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getManagedBotAccessSettings();
```
...
```typescript
const chatinvitelink = new ChatInviteLink(rawData, bot);
await chatinvitelink.getChatMenuButton();
```
...
You can listen for ChatInviteLink events using:

```typescript
// Type-specific handler
bot.onChatInviteLink(async (chatinvitelink: ChatInviteLink) => {
  console.log('Received:', chatinvitelink);
  // Use fluent methods
  await chatinvitelink.getUpdates(...);
});

// Generic handler
bot.on('chatinvitelink', async (data: ChatInviteLink) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatInviteLink).
