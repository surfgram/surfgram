...
| messageId | `number` | Yes | Unique identifier of the message inside the chat |
| user | `User` | No | Optional. The user that changed the reaction, if the user isn't anonymous |
| actorChat | `Chat` | No | Optional. The chat on behalf of which the reaction was changed, if the user is anonymous |
| date | `number` | Yes | Date of the change in Unix time |
| oldReaction | `ReactionType[]` | Yes | Previous list of reaction types that were set by the user |
| newReaction | `ReactionType[]` | Yes | New list of reaction types that have been set by the user |

## Fluent Methods

The `MessageReactionUpdated` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendMessage({
  text: "example text",
...
```typescript
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendMessage({ text: "Response" });
});
```
...
| `fromChatId` | `this.chat?.id` | Unique identifier for the chat where the original message was sent (or username of the target bot, supergroup or channel in the format @username) |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.forwardMessage({
  messageThreadId: 123,
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.forwardMessage({ messageEffectId: "Response" });
});
```
...
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username |
| `fromChatId` | `this.chat?.id` | Unique identifier for the chat where the original messages were sent (or username of the target bot, supergroup or channel in the format @username) |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.forwardMessages({
  messageIds: [123],
  messageThreadId: 123,
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendAudio({ audio: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendDocument({ document: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendVideo({ video: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendVoice({ voice: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendVideoNote({ videoNote: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. If the chat is a channel, all Telegram Star proceeds from this media will be credited to the chat's balance. Otherwise, they will be credited to the bot's balance. |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendVenue({ title: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendContact({
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendContact({ phoneNumber: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. Polls can't be sent to channel direct messages chats. |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendPoll({
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendPoll({ question: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot in the format @username |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendDice({
  businessConnectionId: "example text",
  messageThreadId: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendMessageDraft({ text: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot or supergroup in the format @username. Channel chats and channel direct messages chats aren't supported. |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendChatAction(
  "example text",
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target channel in the format @username |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editChatInviteLink({
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editChatInviteLink({ inviteLink: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editChatSubscriptionInviteLink(
  "example text",
  "example text",
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendChatJoinRequestWebApp(
  "example text",
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editForumTopic(
  123,
  "example text",
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editGeneralForumTopic(
  "example text",
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendGift({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendGift({ giftId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editStory({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Required if inline_message_id is not specified. Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageText({
  businessConnectionId: "example text",
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageCaption({
  businessConnectionId: "example text",
  inlineMessageId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editMessageReplyMarkup({
  businessConnectionId: "example text",
  inlineMessageId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target supergroup in the format @username |
| `receiverUserId` | `this.user?.id` | Identifier of the user who received the message |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editEphemeralMessageText({
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editEphemeralMessageText({ text: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editEphemeralMessageMedia({
  ephemeralMessageId: 123,
  media: {} as any,
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editEphemeralMessageCaption({
  ephemeralMessageId: 123,
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.editEphemeralMessageCaption({ caption: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editEphemeralMessageReplyMarkup(
  123,
  {} as any,
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendInvoice({
  title: "example text",
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendInvoice({ title: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.user?.id` | Identifier of the user whose subscription will be edited |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.editUserStarSubscription(
  "example text",
  true,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot in the format @username. Games can't be sent to channel direct messages chats and channel chats. |
...
```typescript
const messagereactionupdated = new MessageReactionUpdated(rawData, bot);
await messagereactionupdated.sendGame({
...
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  // Auto-fills parameters from the messagereactionupdated instance
  await messagereactionupdated.sendGame({ gameShortName: "Response" });
});
```
...
You can listen for MessageReactionUpdated events using:

```typescript
// Type-specific handler
bot.onMessageReactionUpdated(async (messagereactionupdated: MessageReactionUpdated) => {
  console.log('Received:', messagereactionupdated);
  // Use fluent methods
  await messagereactionupdated.sendMessage(...);
});

// Generic handler
bot.on('messagereactionupdated', async (data: MessageReactionUpdated) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageReactionUpdated).
