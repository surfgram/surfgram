...
| duration | `number` | No | Optional. Precise duration of the video in seconds; 0-60 |
| coverFrameTimestamp | `number` | No | Optional. Timestamp in seconds of the frame that will be used as the static cover for the story. Defaults to 0.0. |
| isAnimation | `boolean` | No | Optional. Pass True if the video has no sound |

## Fluent Methods

The `InputStoryContentVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendContact({
  chatId: 123,
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendPoll({
  chatId: 123,
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendChecklist({
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputstorycontentvideo = new InputStoryContentVideo(rawData, bot);
await inputstorycontentvideo.sendGame({
  chatId: 123,
...
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  // Auto-fills parameters from the inputstorycontentvideo instance
  await inputstorycontentvideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputStoryContentVideo events using:

```typescript
// Type-specific handler
bot.onInputStoryContentVideo(async (inputstorycontentvideo: InputStoryContentVideo) => {
  console.log('Received:', inputstorycontentvideo);
  // Use fluent methods
  await inputstorycontentvideo.sendMessage(...);
});

// Generic handler
bot.on('inputstorycontentvideo', async (data: InputStoryContentVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputStoryContentVideo).
