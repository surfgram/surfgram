...
| :--- | :--- | :---: | :--- |
| messageId | `number` | Yes | Unique message identifier. In specific instances \(e.g., message containing a video sent to a big chat\), the server might automatically schedule a message instead of sending it immediately. In such cases, this field will be 0 and the relevant message will be unusable until it is actually sent. |

## Fluent Methods

The `MessageId` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.forwardMessages({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendPhoto({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendAudio({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendDocument({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendVideo({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendAnimation({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendVoice({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendLocation({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendVenue({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendContact({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendPoll({ chatId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendDice({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editChatInviteLink({
  chatId: 123,
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editForumTopic(
  123,
  123,
);
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editGeneralForumTopic(
  123,
  "example text",
);
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendGift({ giftId: "Response" });
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `ephemeralMessageId` | `this?.id` | Identifier of the ephemeral message to edit |
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editUserStarSubscription(
  123,
  "example text",
);
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendGame({ chatId: "Response" });
});
```
...
You can listen for MessageId events using:
...
// Generic handler
bot.on('messageid', async (data: MessageId) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageId).
