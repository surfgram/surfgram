...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.sendChatAction(123, 'example text');
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editChatSubscriptionInviteLink(123, 'example text');
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editForumTopic(123, 123);
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editGeneralForumTopic(123, 'example text');
```
...
```typescript
const messageid = new MessageId(rawData, bot);
await messageid.editUserStarSubscription(123, 'example text');
```
...
bot.onMessageId(async (messageid: MessageId) => {
  // Auto-fills parameters from the messageid instance
  await messageid.sendGame({ gameShortName: 'Response' });
});
```
