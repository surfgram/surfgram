...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onVideoChatStarted(async (videochatstarted: VideoChatStarted) => {
  // Auto-fills parameters from the videochatstarted instance
  await videochatstarted.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.getManagedBotToken(
  123,
);
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatStarted(async (videochatstarted: VideoChatStarted) => {
  // Auto-fills parameters from the videochatstarted instance
  await videochatstarted.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatstarted = new VideoChatStarted(rawData, bot);
await videochatstarted.sendRichMessageDraft(
  123,
  {} as any,
);
```
