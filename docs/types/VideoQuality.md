...
The `VideoQuality` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendMessage({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendDocument({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendAnimation({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendLocation({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendContact({
  chatId: 123,
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendContact({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendChecklist({
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendDice({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendGift({ giftId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const videoquality = new VideoQuality(rawData, bot);
await videoquality.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendInvoice({ chatId: "Response" });
});
```
...
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  // Auto-fills parameters from the videoquality instance
  await videoquality.sendGame({ chatId: "Response" });
});
```
...
You can listen for VideoQuality events using:

```typescript
// Type-specific handler
bot.onVideoQuality(async (videoquality: VideoQuality) => {
  console.log('Received:', videoquality);
  // Use fluent methods
  await videoquality.sendMessage(...);
});

// Generic handler
bot.on('videoquality', async (data: VideoQuality) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#VideoQuality).
