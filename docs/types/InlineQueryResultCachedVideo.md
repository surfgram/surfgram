...
```typescript
bot.onInlineQueryResultCachedVideo(
  async (inlinequeryresultcachedvideo: InlineQueryResultCachedVideo) => {
...
```typescript
const inlinequeryresultcachedvideo = new InlineQueryResultCachedVideo(rawData, bot);
await inlinequeryresultcachedvideo.sendChatAction(123, 'example text');
```
