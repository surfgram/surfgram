...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getManagedBotAccessSettings(
  123,
);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getUserGifts({ offset: "Response" });
});
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getChatGifts({ offset: "Response" });
});
```
