...
| addDate        | `number`          |   Yes    | Point in time \(Unix timestamp\) when the chat was boosted                                                                                 |
| expirationDate | `number`          |   Yes    | Point in time \(Unix timestamp\) when the boost will automatically expire, unless the booster's Telegram Premium subscription is prolonged |
| source         | `ChatBoostSource` |   Yes    | Source of the added boost                                                                                                                  |

## Fluent Methods

The `ChatBoost` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getWebhookInfo({
  url: 'example text',
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getMe({ text: 'Response' });
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getUserProfilePhotos(123, 123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getUserProfilePhotos();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getFile('example text');
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getChatAdministrators();
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getChatMemberCount();
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getChatMember(123);
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getForumTopicIconStickers();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getUserChatBoosts(123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getUserChatBoosts();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getBusinessConnection('example text');
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getBusinessConnection();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyCommands({} as any, 'example text');
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyName('example text');
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyDescription('example text');
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getMyDescription();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyShortDescription('example text');
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getMyShortDescription();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getChatMenuButton();
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyDefaultAdministratorRights(true);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getAvailableGifts({
  giftId: 'example text',
  userId: 123,
});
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getBusinessAccountStarBalance('example text');
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getStickerSet('example text');
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getCustomEmojiStickers(['example text']);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getCustomEmojiStickers();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getMyStarBalance(123, 123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getMyStarBalance();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getStarTransactions(123, 123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getStarTransactions();
});
```
...
```typescript
const chatboost = new ChatBoost(rawData, bot);
await chatboost.getGameHighScores(123, 123);
```
...
bot.onChatBoost(async (chatboost: ChatBoost) => {
  // Auto-fills parameters from the chatboost instance
  await chatboost.getGameHighScores();
});
```
...
You can listen for ChatBoost events using:

```typescript
// Type-specific handler
bot.onChatBoost(async (chatboost: ChatBoost) => {
  console.log('Received:', chatboost);
  // Use fluent methods
  await chatboost.getUpdates(...);
});

// Generic handler
bot.on('chatboost', async (data: ChatBoost) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatBoost).
