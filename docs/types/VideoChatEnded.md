...
The `VideoChatEnded` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getUpdates({});
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getWebhookInfo({
  url: "example text",
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getWebhookInfo({ url: "Response" });
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getMe({ text: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendMessage({
  text: "example text",
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendMessage({ text: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendAudio({ audio: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendDocument({ document: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendVideo({ video: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendVoice({ voice: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendVideoNote({ videoNote: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendVenue({ title: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendContact({
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendContact({ phoneNumber: "Response" });
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendPoll({ question: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendChatAction(
  "example text",
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getChatAdministrators(
  true,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getChatMemberCount();
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getChatMember(
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUserChatBoosts(
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getBusinessConnection(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getManagedBotToken(
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyName(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyDescription(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyShortDescription(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getChatMenuButton();
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyDefaultAdministratorRights(
  true,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getAvailableGifts({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getAvailableGifts({ giftId: "Response" });
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendGift({ giftId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getUserGifts({
  userId: 123,
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getStickerSet(
  "example text",
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.sendInvoice({
  title: "example text",
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendInvoice({ title: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getStarTransactions(
  123,
  123,
);
```
...
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  // Auto-fills parameters from the videochatended instance
  await videochatended.sendGame({ gameShortName: "Response" });
});
```
...
```typescript
const videochatended = new VideoChatEnded(rawData, bot);
await videochatended.getGameHighScores(
  123,
  123,
);
```
...
You can listen for VideoChatEnded events using:

```typescript
// Type-specific handler
bot.onVideoChatEnded(async (videochatended: VideoChatEnded) => {
  console.log('Received:', videochatended);
  // Use fluent methods
  await videochatended.getUpdates(...);
});

// Generic handler
bot.on('videochatended', async (data: VideoChatEnded) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#VideoChatEnded).
