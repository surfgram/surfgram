...
The `MessageEntity` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.getMe({
  chatId: 123,
  text: "example text",
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.getMe({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.logOut({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.close({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.copyMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.copyMessage({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendDocument({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendAnimation({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendLocation({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendContact({
  chatId: 123,
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendContact({ chatId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendChecklist({
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendDice({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editChatInviteLink({
  chatId: 123,
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editForumTopic(
  123,
  123,
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.getAvailableGifts({
  giftId: "example text",
  chatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.getAvailableGifts({ giftId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendGift({ giftId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.user?.id` | Unique identifier of the target user who will receive a Telegram Premium subscription |
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.giftPremiumSubscription({ text: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.postStory({ businessConnectionId: "Response" });
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editEphemeralMessageText({
  chatId: 123,
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editEphemeralMessageMedia({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editEphemeralMessageCaption({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const messageentity = new MessageEntity(rawData, bot);
await messageentity.editUserStarSubscription(
  "example text",
  true,
);
```
...
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  // Auto-fills parameters from the messageentity instance
  await messageentity.sendGame({ chatId: "Response" });
});
```
...
You can listen for MessageEntity events using:

```typescript
// Type-specific handler
bot.onMessageEntity(async (messageentity: MessageEntity) => {
  console.log('Received:', messageentity);
  // Use fluent methods
  await messageentity.getMe(...);
});

// Generic handler
bot.on('messageentity', async (data: MessageEntity) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageEntity).
