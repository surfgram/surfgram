...
| :--- | :--- | :---: | :--- |
| messageId | `number` | No | Optional. Identifier of the message that will be replied to in the current chat, or in the chat chat\_id if it is specified. Required if ephemeral\_message\_id isn't specified. |
| chatId | `number` \| `string` | No | Optional. If the message to be replied to is from a different chat, unique identifier for the chat or username of the bot, supergroup or channel in the format @username. Not supported for messages sent on behalf of a business account, messages from channel direct messages chats and ephemeral messages. |
| ephemeralMessageId | `number` | No | Optional. Identifier of the incoming ephemeral message that will be replied to in the current chat. A reply to an ephemeral message must itself be an ephemeral message. An ephemeral message may only be replied to within 15 seconds of being sent. Required if message\_id isn't specified. |
| allowSendingWithoutReply | `boolean` | No | Optional. Pass True if the message should be sent even if the specified message to be replied to is not found. Always False for replies in another chat or forum topic, and sent ephemeral messages. Always True for messages sent on behalf of a business account. |
...
The `ReplyParameters` class has the following fluent methods that automatically inject contextual parameters:
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  ...
  await replyparameters.getMe({ text: "Response" });
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.logOut({ text: "Response" });
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.close({ text: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendMessage({
  text: "example text",
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendMessage({ text: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.copyMessage({
  fromChatId: 123,
  messageThreadId: 123,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.copyMessage({ fromChatId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendAudio({ audio: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendDocument({ document: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendVideo({ video: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendVoice({ voice: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendVideoNote({ videoNote: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chatId` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. If the chat is a channel, all Telegram Star proceeds from this media will be credited to the chat's balance. Otherwise, they will be credited to the bot's balance. |
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendVenue({ title: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendContact({
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendContact({ phoneNumber: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chatId` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. Polls can't be sent to channel direct messages chats. |
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendPoll({
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendPoll({ question: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chatId` | Unique identifier for the target chat or username of the target bot in the format @username |
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendDice({
  businessConnectionId: "example text",
  messageThreadId: 123,
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendInvoice({
  title: "example text",
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendInvoice({ title: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.chatId` | Unique identifier for the target chat or username of the target bot in the format @username. Games can't be sent to channel direct messages chats and channel chats. |
...
```typescript
const replyparameters = new ReplyParameters(rawData, bot);
await replyparameters.sendGame({
...
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  // Auto-fills parameters from the replyparameters instance
  await replyparameters.sendGame({ gameShortName: "Response" });
});
```
...
You can listen for ReplyParameters events using:

```typescript
// Type-specific handler
bot.onReplyParameters(async (replyparameters: ReplyParameters) => {
  console.log('Received:', replyparameters);
  // Use fluent methods
  await replyparameters.getMe(...);
});

// Generic handler
bot.on('replyparameters', async (data: ReplyParameters) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ReplyParameters).
