...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.setChatMemberTag(
  "example text",
);
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getManagedBotToken();
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getUserGifts({ offset: "Response" });
});
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getChatGifts({ offset: "Response" });
});
```
