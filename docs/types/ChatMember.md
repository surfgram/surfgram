...
The `ChatMember` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getWebhookInfo({
  url: 'example text',
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMe({ text: 'Response' });
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getUserProfilePhotos(123, 123);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getUserProfilePhotos();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getFile('example text');
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.banChatMember(123, true);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.banChatMember();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.unbanChatMember(true);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.unbanChatMember();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.restrictChatMember({
  permissions: {} as any,
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.promoteChatMember({
  isAnonymous: true,
  canManageChat: true,
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getChatMemberCount();
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getChatMember();
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getForumTopicIconStickers();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getUserChatBoosts();
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getBusinessConnection('example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getBusinessConnection();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyCommands({} as any, 'example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMyCommands();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyName('example text');
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyDescription('example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMyDescription();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyShortDescription('example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMyShortDescription();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getChatMenuButton();
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyDefaultAdministratorRights(true);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getAvailableGifts({
  giftId: 'example text',
  payForUpgrade: true,
});
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getBusinessAccountStarBalance('example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getStickerSet('example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getStickerSet();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getCustomEmojiStickers(['example text']);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getCustomEmojiStickers();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getMyStarBalance(123, 123);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getMyStarBalance();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getStarTransactions(123, 123);
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getStarTransactions();
});
```
...
```typescript
const chatmember = new ChatMember(rawData, bot);
await chatmember.getGameHighScores(123, 'example text');
```
...
bot.onChatMember(async (chatmember: ChatMember) => {
  // Auto-fills parameters from the chatmember instance
  await chatmember.getGameHighScores();
});
```
...
You can listen for ChatMember events using:

```typescript
// Type-specific handler
bot.onChatMember(async (chatmember: ChatMember) => {
  console.log('Received:', chatmember);
  // Use fluent methods
  await chatmember.getUpdates(...);
});

// Generic handler
bot.on('chatmember', async (data: ChatMember) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatMember).
