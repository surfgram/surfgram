...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getManagedBotToken();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getManagedBotAccessSettings();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUserGifts({
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getChatGifts({ offset: "Response" });
});
```
