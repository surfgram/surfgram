...
| untilDate | `number` |   Yes    | Date when restrictions will be lifted for this user; Unix time. If 0, then the user is banned forever |

## Fluent Methods

The `ChatMemberBanned` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getUpdates({});
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getWebhookInfo({
  url: 'example text',
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMe({ text: 'Response' });
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUserProfilePhotos(123, 123);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getUserProfilePhotos();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getFile('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getFile();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChat();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChatAdministrators();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChatMemberCount();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChatMember();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getForumTopicIconStickers();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getUserChatBoosts();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getBusinessConnection('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getBusinessConnection();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyCommands({} as any, 'example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyCommands();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyName('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyName();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyDescription('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyDescription();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyShortDescription('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyShortDescription();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getChatMenuButton();
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyDefaultAdministratorRights(true);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getAvailableGifts({
  giftId: 'example text',
  payForUpgrade: true,
});
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getBusinessAccountStarBalance('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getStickerSet('example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getStickerSet();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getCustomEmojiStickers(['example text']);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getCustomEmojiStickers();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getMyStarBalance(123, 123);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getMyStarBalance();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getStarTransactions(123, 123);
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getStarTransactions();
});
```
...
```typescript
const chatmemberbanned = new ChatMemberBanned(rawData, bot);
await chatmemberbanned.getGameHighScores(123, 'example text');
```
...
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  // Auto-fills parameters from the chatmemberbanned instance
  await chatmemberbanned.getGameHighScores();
});
```
...
You can listen for ChatMemberBanned events using:

```typescript
// Type-specific handler
bot.onChatMemberBanned(async (chatmemberbanned: ChatMemberBanned) => {
  console.log('Received:', chatmemberbanned);
  // Use fluent methods
  await chatmemberbanned.getUpdates(...);
});

// Generic handler
bot.on('chatmemberbanned', async (data: ChatMemberBanned) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatMemberBanned).
