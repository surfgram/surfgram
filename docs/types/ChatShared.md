...
| chatId    | `number`      |   Yes    | Identifier of the shared chat. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier. The bot may not have access to the chat and could be unable to use this identifier, unless the chat is already known to the bot by some other means. |
| title     | `string`      |    No    | Optional. Title of the chat, if the title was requested by the bot.                                                                                                                                                                                                                                                                                                                                                                                           |
| username  | `string`      |    No    | Optional. Username of the chat, if the username was requested by the bot and available.                                                                                                                                                                                                                                                                                                                                                                       |
| photo     | `PhotoSize[]` |    No    | Optional. Available sizes of the chat photo, if the photo was requested by the bot                                                                                                                                                                                                                                                                                                                                                                            |

## Fluent Methods

The `ChatShared` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getUpdates({});
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getWebhookInfo({
  url: 'example text',
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMe({ text: 'Response' });
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getUserProfilePhotos(123, 123);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getUserProfilePhotos();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getFile('example text');
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getChatAdministrators();
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getChatMemberCount();
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getChatMember();
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getForumTopicIconStickers();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getUserChatBoosts();
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getBusinessConnection('example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getBusinessConnection();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyCommands({} as any, 'example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMyCommands();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyName('example text');
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyDescription('example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMyDescription();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyShortDescription('example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMyShortDescription();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getChatMenuButton();
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyDefaultAdministratorRights(true);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getAvailableGifts({
  giftId: 'example text',
  payForUpgrade: true,
});
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getBusinessAccountStarBalance('example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getStickerSet('example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getStickerSet();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getCustomEmojiStickers(['example text']);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getCustomEmojiStickers();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getMyStarBalance(123, 123);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getMyStarBalance();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getStarTransactions(123, 123);
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getStarTransactions();
});
```
...
```typescript
const chatshared = new ChatShared(rawData, bot);
await chatshared.getGameHighScores(123, 'example text');
```
...
bot.onChatShared(async (chatshared: ChatShared) => {
  // Auto-fills parameters from the chatshared instance
  await chatshared.getGameHighScores();
});
```
...
You can listen for ChatShared events using:

```typescript
// Type-specific handler
bot.onChatShared(async (chatshared: ChatShared) => {
  console.log('Received:', chatshared);
  // Use fluent methods
  await chatshared.getUpdates(...);
});

// Generic handler
bot.on('chatshared', async (data: ChatShared) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatShared).
