...
The `VideoNote` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendMessage({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendPhoto({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendAudio({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendDocument({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendVideo({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendAnimation({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendVoice({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendLocation({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendVenue({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendContact({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendPoll({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendDice({ chatId: "Response" });
});
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendGift({ giftId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const videonote = new VideoNote(rawData, bot);
await videonote.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendInvoice({ chatId: "Response" });
});
```
...
bot.onVideoNote(async (videonote: VideoNote) => {
  // Auto-fills parameters from the videonote instance
  await videonote.sendGame({ chatId: "Response" });
});
```
...
You can listen for VideoNote events using:

```typescript
// Type-specific handler
bot.onVideoNote(async (videonote: VideoNote) => {
  console.log('Received:', videonote);
  // Use fluent methods
  await videonote.sendMessage(...);
});

// Generic handler
bot.on('videonote', async (data: VideoNote) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#VideoNote).
