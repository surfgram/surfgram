...
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username |
...
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username |
| `fromChatId` | `this.senderChat?.id` | Unique identifier for the chat where the original message was sent (or username of the target bot, supergroup or channel in the format @username) |
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.forwardMessage({ messageEffectId: "Response" });
});
```
...
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username |
| `fromChatId` | `this.senderChat?.id` | Unique identifier for the chat where the original messages were sent (or username of the target bot, supergroup or channel in the format @username) |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.sendLivePhoto({ livePhoto: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. If the chat is a channel, all Telegram Star proceeds from this media will be credited to the chat's balance. Otherwise, they will be credited to the bot's balance. |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. Polls can't be sent to channel direct messages chats. |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot in the format @username |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target private chat |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.sendMessageDraft({ text: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot or supergroup in the format @username. Channel chats and channel direct messages chats aren't supported. |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getUserProfileAudios(
  123,
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target channel in the format @username |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatJoinRequestQueryId` | `this.senderChat?.id` | Unique identifier of the join request query |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.sendChatJoinRequestWebApp(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target supergroup or channel in the format @username |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getUserPersonalChatMessages(
  123,
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target supergroup in the format @username |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the chat or username of the channel in the format @username |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getManagedBotToken(
  123,
);
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getManagedBotAccessSettings(
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target private chat. If not specified, the bot's default menu button will be returned. |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Required if user_id is not specified. Unique identifier for the chat or username of the channel (in the format @username) that will receive the gift. |
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getUserGifts({
  userId: 123,
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.getChatGifts({ offset: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Required if inline_message_id is not specified. Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. |
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.editEphemeralMessageText({
  receiverUserId: 123,
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.editEphemeralMessageText({ text: "Response" });
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.editEphemeralMessageMedia({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.editEphemeralMessageCaption({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.editEphemeralMessageCaption({ caption: "Response" });
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginChat(async (messageoriginchat: MessageOriginChat) => {
  // Auto-fills parameters from the messageoriginchat instance
  await messageoriginchat.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginchat = new MessageOriginChat(rawData, bot);
await messageoriginchat.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.senderChat?.id` | Unique identifier for the target chat or username of the target bot in the format @username. Games can't be sent to channel direct messages chats and channel chats. |
