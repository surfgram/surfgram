...
| :--- | :--- | :---: | :--- |
| newOwner | `User` | Yes | The new owner of the chat |

## Fluent Methods

The `ChatOwnerChanged` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getUpdates({});
});
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getWebhookInfo({
  url: "example text",
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getWebhookInfo({ url: "Response" });
});
```
...
| :--- | :--- | :--- |
| `receiverUserId` | `this.newOwner?.id` | For outgoing ephemeral messages, unique identifier of the user who will receive the message; for group and supergroup chats only. It is not guaranteed that the user will receive the message, especially if they are offline. See ephemeral message sending for more details. |
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getMe({ text: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | Unique identifier of the target user |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getFile(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChat();
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChatAdministrators(
  true,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChatMemberCount();
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChatMember();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | Unique identifier for the target user |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUserChatBoosts();
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getBusinessConnection(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | User identifier of the managed bot whose token will be returned |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getManagedBotToken();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | User identifier of the managed bot whose access settings will be returned |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getManagedBotAccessSettings();
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyName(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyDescription(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyShortDescription(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChatMenuButton();
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyDefaultAdministratorRights(
  true,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | Required if chat_id is not specified. Unique identifier of the target user who will receive the gift. |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getAvailableGifts({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | Unique identifier of the user |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getUserGifts({
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  // Auto-fills parameters from the chatownerchanged instance
  await chatownerchanged.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getStickerSet(
  "example text",
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getStarTransactions(
  123,
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.newOwner?.id` | Target user id |
...
```typescript
const chatownerchanged = new ChatOwnerChanged(rawData, bot);
await chatownerchanged.getGameHighScores(
  123,
  "example text",
);
```
...
You can listen for ChatOwnerChanged events using:

```typescript
// Type-specific handler
bot.onChatOwnerChanged(async (chatownerchanged: ChatOwnerChanged) => {
  console.log('Received:', chatownerchanged);
  // Use fluent methods
  await chatownerchanged.getUpdates(...);
});

// Generic handler
bot.on('chatownerchanged', async (data: ChatOwnerChanged) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatOwnerChanged).
