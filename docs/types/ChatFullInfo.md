...
| hasPrivateForwards | `boolean` | No | Optional. True, if privacy settings of the other party in the private chat allows to use tg://user?id=&lt;user\_id&gt; links only in chats with the user |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.personalChat?.id` | Unique identifier for the target chat or username of the target supergroup or channel in the format @username |
...
```typescript
const chatfullinfo = new ChatFullInfo(rawData, bot);
await chatfullinfo.getUserPersonalChatMessages(
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.personalChat?.id` | Unique identifier for the target chat or username of the target supergroup in the format @username |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.personalChat?.id` | Unique identifier for the chat or username of the channel in the format @username |
| `userId` | `this.username?.id` | Unique identifier of the target user |
...
```typescript
const chatfullinfo = new ChatFullInfo(rawData, bot);
await chatfullinfo.getManagedBotToken();
```
...
```typescript
const chatfullinfo = new ChatFullInfo(rawData, bot);
await chatfullinfo.getManagedBotAccessSettings();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.personalChat?.id` | Unique identifier for the target private chat. If not specified, the bot's default menu button will be returned. |
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `giftId` | `this.acceptedGiftTypes?.id` | Identifier of the gift; limited gifts can't be sent to channel chats |
...
| `chatId` | `this.personalChat?.id` | Required if user_id is not specified. Unique identifier for the chat or username of the channel (in the format @username) that will receive the gift. |
...
```typescript
const chatfullinfo = new ChatFullInfo(rawData, bot);
await chatfullinfo.getUserGifts({
...
bot.onChatFullInfo(async (chatfullinfo: ChatFullInfo) => {
  // Auto-fills parameters from the chatfullinfo instance
  await chatfullinfo.getUserGifts({ offset: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.personalChat?.id` | Unique identifier for the target chat or username of the target channel in the format @username |
...
```typescript
const chatfullinfo = new ChatFullInfo(rawData, bot);
await chatfullinfo.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatFullInfo(async (chatfullinfo: ChatFullInfo) => {
  // Auto-fills parameters from the chatfullinfo instance
  await chatfullinfo.getChatGifts({ offset: "Response" });
});
```
