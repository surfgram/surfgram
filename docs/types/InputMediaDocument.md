...
| disableContentTypeDetection | `boolean` | No | Optional. Disables automatic server-side content type detection for files uploaded using multipart/form-data. Always True, if the document is sent as part of an album. |

## Fluent Methods

The `InputMediaDocument` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendContact({
  chatId: 123,
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendPoll({
  chatId: 123,
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendChecklist({
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputmediadocument = new InputMediaDocument(rawData, bot);
await inputmediadocument.sendGame({
  chatId: 123,
...
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  // Auto-fills parameters from the inputmediadocument instance
  await inputmediadocument.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputMediaDocument events using:

```typescript
// Type-specific handler
bot.onInputMediaDocument(async (inputmediadocument: InputMediaDocument) => {
  console.log('Received:', inputmediadocument);
  // Use fluent methods
  await inputmediadocument.sendMessage(...);
});

// Generic handler
bot.on('inputmediadocument', async (data: InputMediaDocument) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputMediaDocument).
