...
| :--- | :--- | :---: | :--- |
| type | `string` | Yes | Type of the message origin, always “hidden\_user” |
| date | `number` | Yes | Date the message was sent originally in Unix time |
| senderUserName | `string` | Yes | Name of the user that sent the message originally |

## Fluent Methods

The `MessageOriginHiddenUser` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendContact({
  chatId: 123,
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendContact({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendPoll({
  chatId: 123,
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendChecklist({
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendDice({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editChatInviteLink({
  chatId: 123,
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editForumTopic(
  123,
  123,
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editGeneralForumTopic(
  123,
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.senderUserName?.id` | Required if chat_id is not specified. Unique identifier of the target user who will receive the gift. |
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendGift({
  giftId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendGift({ giftId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendInvoice({ chatId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.senderUserName?.id` | Identifier of the user whose subscription will be edited |
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.editUserStarSubscription(
  "example text",
  true,
);
```
...
```typescript
const messageoriginhiddenuser = new MessageOriginHiddenUser(rawData, bot);
await messageoriginhiddenuser.sendGame({
  chatId: 123,
...
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  // Auto-fills parameters from the messageoriginhiddenuser instance
  await messageoriginhiddenuser.sendGame({ chatId: "Response" });
});
```
...
You can listen for MessageOriginHiddenUser events using:

```typescript
// Type-specific handler
bot.onMessageOriginHiddenUser(async (messageoriginhiddenuser: MessageOriginHiddenUser) => {
  console.log('Received:', messageoriginhiddenuser);
  // Use fluent methods
  await messageoriginhiddenuser.sendMessage(...);
});

// Generic handler
bot.on('messageoriginhiddenuser', async (data: MessageOriginHiddenUser) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageOriginHiddenUser).
