...
| photoFileId | `string` | Yes | A valid file identifier of the photo |
...
| inputMessageContent | `InputMessageContent` | No | Optional. Content of the message to be sent instead of the photo |

## Fluent Methods

The `InlineQueryResultCachedPhoto` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendContact({
  chatId: 123,
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendPoll({
  chatId: 123,
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendChecklist({
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendGame({
  chatId: 123,
...
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  // Auto-fills parameters from the inlinequeryresultcachedphoto instance
  await inlinequeryresultcachedphoto.sendGame({ chatId: "Response" });
});
```
...
You can listen for InlineQueryResultCachedPhoto events using:

```typescript
// Type-specific handler
bot.onInlineQueryResultCachedPhoto(async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
  console.log('Received:', inlinequeryresultcachedphoto);
  // Use fluent methods
  await inlinequeryresultcachedphoto.sendMessage(...);
});

// Generic handler
bot.on('inlinequeryresultcachedphoto', async (data: InlineQueryResultCachedPhoto) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InlineQueryResultCachedPhoto).
