...
```typescript
bot.onInlineQueryResultCachedPhoto(
  async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
...
```typescript
const inlinequeryresultcachedphoto = new InlineQueryResultCachedPhoto(rawData, bot);
await inlinequeryresultcachedphoto.sendChatAction(123, 'example text');
```
...
bot.onInlineQueryResultCachedPhoto(
  async (inlinequeryresultcachedphoto: InlineQueryResultCachedPhoto) => {
    // Auto-fills parameters from the inlinequeryresultcachedphoto instance
    await inlinequeryresultcachedphoto.sendGame({ gameShortName: 'Response' });
  }
);
```
