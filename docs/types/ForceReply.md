...
| :--- | :--- | :---: | :--- |
| forceReply | `boolean` | Yes | Shows reply interface to the user, as if they manually selected the bot's message and tapped 'Reply' |
| inputFieldPlaceholder | `string` | No | Optional. The placeholder to be shown in the input field when the reply is active; 1-64 characters |
| selective | `boolean` | No | Optional. Use this parameter if you want to force reply from specific users only. Targets: 1\) users that are @mentioned in the text of the Message object; 2\) if the bot's message is a reply to a message in the same chat and forum topic, sender of the original message. |

## Fluent Methods

The `ForceReply` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.getMe({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.getMe({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.logOut({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.close({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendMessage({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.copyMessage({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendAudio({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendDocument({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendAnimation({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendPaidMedia({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendLocation({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendVenue({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendContact({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendPoll({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendDice({ chatId: "Response" });
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const forcereply = new ForceReply(rawData, bot);
await forcereply.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onForceReply(async (forcereply: ForceReply) => {
  // Auto-fills parameters from the forcereply instance
  await forcereply.sendRichMessage({ chatId: "Response" });
});
```
...
You can listen for ForceReply events using:

```typescript
// Type-specific handler
bot.onForceReply(async (forcereply: ForceReply) => {
  console.log('Received:', forcereply);
  // Use fluent methods
  await forcereply.getMe(...);
});

// Generic handler
bot.on('forcereply', async (data: ForceReply) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ForceReply).
