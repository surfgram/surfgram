...
The `ChatAdministratorRights` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getUpdates({});
});
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getWebhookInfo({
  url: "example text",
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getWebhookInfo({ url: "Response" });
});
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMe({
  text: "example text",
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getMe({ text: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | Unique identifier of the target user |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getFile(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.canManageChat?.id` | Unique identifier for the target chat or username of the target supergroup or channel in the format @username |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChat();
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChatAdministrators(
  true,
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChatMemberCount();
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChatMember();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | Unique identifier for the target user |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUserPersonalChatMessages(
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.canManageChat?.id` | Unique identifier for the target chat or username of the target supergroup in the format @username |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.canManageChat?.id` | Unique identifier for the chat or username of the channel in the format @username |
| `userId` | `this.canInviteUsers?.id` | Unique identifier of the target user |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUserChatBoosts();
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getBusinessConnection(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | User identifier of the managed bot whose token will be returned |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getManagedBotToken();
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | User identifier of the managed bot whose access settings will be returned |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getManagedBotAccessSettings();
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyName(
  "example text",
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyDescription(
  "example text",
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyShortDescription(
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.canManageChat?.id` | Unique identifier for the target private chat. If not specified, the bot's default menu button will be returned. |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChatMenuButton();
```
...
**See also:** [setMyDefaultAdministratorRights API method](../methods/setMyDefaultAdministratorRights.md)
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyDefaultAdministratorRights(
  true,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | Required if chat_id is not specified. Unique identifier of the target user who will receive the gift. |
| `chatId` | `this.canManageChat?.id` | Required if user_id is not specified. Unique identifier for the chat or username of the channel (in the format @username) that will receive the gift. |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getAvailableGifts({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | Unique identifier of the user |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getUserGifts({
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getUserGifts({ offset: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `chatId` | `this.canManageChat?.id` | Unique identifier for the target chat or username of the target channel in the format @username |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  // Auto-fills parameters from the chatadministratorrights instance
  await chatadministratorrights.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getStickerSet(
  "example text",
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getStarTransactions(
  123,
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.canInviteUsers?.id` | Target user id |
...
```typescript
const chatadministratorrights = new ChatAdministratorRights(rawData, bot);
await chatadministratorrights.getGameHighScores(
  "example text",
);
```
...
You can listen for ChatAdministratorRights events using:

```typescript
// Type-specific handler
bot.onChatAdministratorRights(async (chatadministratorrights: ChatAdministratorRights) => {
  console.log('Received:', chatadministratorrights);
  // Use fluent methods
  await chatadministratorrights.getUpdates(...);
});

// Generic handler
bot.on('chatadministratorrights', async (data: ChatAdministratorRights) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatAdministratorRights).
