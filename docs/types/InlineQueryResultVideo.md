...
| videoUrl | `string` | Yes | A valid URL for the embedded video player or video file |
| mimeType | `string` | Yes | MIME type of the content of the video URL, “text/html” or “video/mp4” |
| thumbnailUrl | `string` | Yes | URL of the thumbnail \(JPEG only\) for the video |
...
| videoWidth | `number` | No | Optional. Video width |
| videoHeight | `number` | No | Optional. Video height |
| videoDuration | `number` | No | Optional. Video duration in seconds |
...
| inputMessageContent | `InputMessageContent` | No | Optional. Content of the message to be sent instead of the video. This field is required if InlineQueryResultVideo is used to send an HTML-page as a result \(e.g., a YouTube video\). |

## Fluent Methods

The `InlineQueryResultVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendContact({
  chatId: 123,
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendPoll({
  chatId: 123,
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendChecklist({
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inlinequeryresultvideo = new InlineQueryResultVideo(rawData, bot);
await inlinequeryresultvideo.sendGame({
  chatId: 123,
...
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  // Auto-fills parameters from the inlinequeryresultvideo instance
  await inlinequeryresultvideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for InlineQueryResultVideo events using:

```typescript
// Type-specific handler
bot.onInlineQueryResultVideo(async (inlinequeryresultvideo: InlineQueryResultVideo) => {
  console.log('Received:', inlinequeryresultvideo);
  // Use fluent methods
  await inlinequeryresultvideo.sendMessage(...);
});

// Generic handler
bot.on('inlinequeryresultvideo', async (data: InlineQueryResultVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InlineQueryResultVideo).
