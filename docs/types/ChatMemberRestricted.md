...
| canReactToMessages | `boolean` | Yes | True, if the user is allowed to react to messages |
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getUserPersonalChatMessages(
  123,
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.user?.id` | User identifier of the managed bot whose token will be returned |
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getManagedBotToken();
```
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getManagedBotAccessSettings();
```
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getUserGifts({
...
bot.onChatMemberRestricted(async (chatmemberrestricted: ChatMemberRestricted) => {
  // Auto-fills parameters from the chatmemberrestricted instance
  await chatmemberrestricted.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatmemberrestricted = new ChatMemberRestricted(rawData, bot);
await chatmemberrestricted.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatMemberRestricted(async (chatmemberrestricted: ChatMemberRestricted) => {
  // Auto-fills parameters from the chatmemberrestricted instance
  await chatmemberrestricted.getChatGifts({ offset: "Response" });
});
```
