...
| :--- | :--- | :---: | :--- |
| newOwner | `User` | No | Optional. The user who will become the new owner of the chat if the previous owner does not return to the chat |

## Fluent Methods

The `ChatOwnerLeft` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getUpdates({});
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getWebhookInfo({
  url: "example text",
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getWebhookInfo({ url: "Response" });
});
```
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getMe({ text: "Response" });
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getChatAdministrators(
  true,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getChatMemberCount();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getChatMember();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUserChatBoosts();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getBusinessConnection(
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getManagedBotToken();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getManagedBotAccessSettings();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getMyDescription(
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getMyShortDescription(
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getChatMenuButton();
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getMyDefaultAdministratorRights(
  true,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getAvailableGifts({
  giftId: "example text",
  payForUpgrade: true,
});
```
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getUserGifts({
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  // Auto-fills parameters from the chatownerleft instance
  await chatownerleft.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getStickerSet(
  "example text",
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getStarTransactions(
  123,
  123,
);
```
...
```typescript
const chatownerleft = new ChatOwnerLeft(rawData, bot);
await chatownerleft.getGameHighScores(
  123,
  "example text",
);
```
...
You can listen for ChatOwnerLeft events using:

```typescript
// Type-specific handler
bot.onChatOwnerLeft(async (chatownerleft: ChatOwnerLeft) => {
  console.log('Received:', chatownerleft);
  // Use fluent methods
  await chatownerleft.getUpdates(...);
});

// Generic handler
bot.on('chatownerleft', async (data: ChatOwnerLeft) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatOwnerLeft).
