...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getManagedBotToken(
  123,
);
```
...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getUserGifts({
  userId: 123,
...
bot.onChatBoostRemoved(async (chatboostremoved: ChatBoostRemoved) => {
  // Auto-fills parameters from the chatboostremoved instance
  await chatboostremoved.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostremoved = new ChatBoostRemoved(rawData, bot);
await chatboostremoved.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostRemoved(async (chatboostremoved: ChatBoostRemoved) => {
  // Auto-fills parameters from the chatboostremoved instance
  await chatboostremoved.getChatGifts({ offset: "Response" });
});
```
