...
You can listen for SharedUser events using:

```typescript
// Type-specific handler
bot.onSharedUser(async (shareduser: SharedUser) => {
  console.log('Received:', shareduser);
});

// Generic handler
bot.on('shareduser', async (data: SharedUser) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#SharedUser).
