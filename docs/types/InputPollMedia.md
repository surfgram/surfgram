...
The `InputPollMedia` class has the following fluent methods that automatically inject contextual parameters:
...
| :--- | :--- | :--- |
| `chatId` | `this.addedByChat?.id` | Unique identifier for the target chat or username of the target bot, supergroup or channel in the format @username. Polls can't be sent to channel direct messages chats. |
...
```typescript
const inputpollmedia = new InputPollMedia(rawData, bot);
await inputpollmedia.sendPoll({
...
```typescript
bot.onInputPollMedia(async (inputpollmedia: InputPollMedia) => {
  // Auto-fills parameters from the inputpollmedia instance
  await inputpollmedia.sendPoll({ question: "Response" });
});
```
...
You can listen for InputPollMedia events using:

```typescript
// Type-specific handler
bot.onInputPollMedia(async (inputpollmedia: InputPollMedia) => {
  console.log('Received:', inputpollmedia);
  // Use fluent methods
  await inputpollmedia.sendPoll(...);
});

// Generic handler
bot.on('inputpollmedia', async (data: InputPollMedia) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputPollMedia).
