...
The `MessageOrigin` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.forwardMessages({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendDocument({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendAnimation({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendLocation({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendContact({
  chatId: 123,
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendContact({ chatId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendChecklist({
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendDice({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editChatInviteLink({
  chatId: 123,
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editForumTopic(
  123,
  123,
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editGeneralForumTopic(
  123,
  "example text",
);
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendGift({ giftId: "Response" });
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editEphemeralMessageText({
  chatId: 123,
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editEphemeralMessageMedia({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editEphemeralMessageCaption({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const messageorigin = new MessageOrigin(rawData, bot);
await messageorigin.editUserStarSubscription(
  "example text",
  true,
);
```
...
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  // Auto-fills parameters from the messageorigin instance
  await messageorigin.sendGame({ chatId: "Response" });
});
```
...
You can listen for MessageOrigin events using:

```typescript
// Type-specific handler
bot.onMessageOrigin(async (messageorigin: MessageOrigin) => {
  console.log('Received:', messageorigin);
  // Use fluent methods
  await messageorigin.sendMessage(...);
});

// Generic handler
bot.on('messageorigin', async (data: MessageOrigin) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageOrigin).
