...
| :--- | :--- | :--- |
| `fromChatId` | `this.chat?.id` | Unique identifier of the chat which posted the story that should be reposted |
| `fromStoryId` | `this?.id` | Unique identifier of the story that should be reposted |
...
bot.onStory(async (story: Story) => {
  // Auto-fills parameters from the story instance
  await story.repostStory({ businessConnectionId: "Response" });
});
```

**See also:** [repostStory API method](../methods/repostStory.md)
