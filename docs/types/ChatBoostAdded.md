...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getManagedBotToken(
  123,
);
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUserGifts({
  userId: 123,
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getChatGifts({ offset: "Response" });
});
```
