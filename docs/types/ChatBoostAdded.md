...
| :--------- | :------- | :------: | :--------------------------------- |
| boostCount | `number` |   Yes    | Number of boosts added by the user |

## Fluent Methods

The `ChatBoostAdded` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getUpdates({});
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getWebhookInfo({
  url: 'example text',
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMe({ text: 'Response' });
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUserProfilePhotos(123, 123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getUserProfilePhotos();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getFile('example text');
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getChatAdministrators();
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getChatMemberCount();
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getChatMember(123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getChatMember();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getForumTopicIconStickers();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getUserChatBoosts(123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getUserChatBoosts();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getBusinessConnection('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getBusinessConnection();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyCommands({} as any, 'example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyCommands();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyName('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyName();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyDescription('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyDescription();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyShortDescription('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyShortDescription();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getChatMenuButton();
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyDefaultAdministratorRights(true);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getAvailableGifts({
  giftId: 'example text',
  userId: 123,
});
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getBusinessAccountStarBalance('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getStickerSet('example text');
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getStickerSet();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getCustomEmojiStickers(['example text']);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getCustomEmojiStickers();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getMyStarBalance(123, 123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getMyStarBalance();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getStarTransactions(123, 123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getStarTransactions();
});
```
...
```typescript
const chatboostadded = new ChatBoostAdded(rawData, bot);
await chatboostadded.getGameHighScores(123, 123);
```
...
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  // Auto-fills parameters from the chatboostadded instance
  await chatboostadded.getGameHighScores();
});
```
...
You can listen for ChatBoostAdded events using:

```typescript
// Type-specific handler
bot.onChatBoostAdded(async (chatboostadded: ChatBoostAdded) => {
  console.log('Received:', chatboostadded);
  // Use fluent methods
  await chatboostadded.getUpdates(...);
});

// Generic handler
bot.on('chatboostadded', async (data: ChatBoostAdded) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatBoostAdded).
