...
| messageIds | `number[]` | Yes | The list of identifiers of deleted messages in the chat of the business account |

## Fluent Methods

The `BusinessMessagesDeleted` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendMessage({
  text: "example text",
  messageThreadId: 123,
});
```
...
```typescript
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendMessage({ text: "Response" });
});
```
...
| `messageId` | `this.messageIds?.id` | Message identifier in the chat specified in from_chat_id |
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.forwardMessage({
  messageThreadId: 123,
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.forwardMessage({ messageEffectId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.forwardMessages({
  messageIds: [123],
  messageThreadId: 123,
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendPhoto({
  photo: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendAudio({
  audio: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendAudio({ audio: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendDocument({
  document: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendDocument({ document: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendVideo({
  video: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendVideo({ video: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendAnimation({
  animation: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendVoice({
  voice: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendVoice({ voice: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendVideoNote({
  videoNote: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendVideoNote({ videoNote: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendPaidMedia({ payload: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendMediaGroup({
  media: [{} as any],
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendMediaGroup({ messageEffectId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendLocation({ callbackQueryId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendVenue({ title: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendContact({
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendContact({ phoneNumber: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendPoll({
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendPoll({ question: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendChecklist({
  checklist: {} as any,
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendChecklist({ messageEffectId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendDice({
  messageThreadId: 123,
  directMessagesTopicId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendDice({ emoji: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendChatAction(
  "example text",
  123,
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editChatInviteLink({
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editChatInviteLink({ inviteLink: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editChatSubscriptionInviteLink(
  "example text",
  "example text",
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendChatJoinRequestWebApp(
  "example text",
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editForumTopic(
  123,
  "example text",
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editGeneralForumTopic(
  "example text",
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendGift({ giftId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editStory({
  storyId: 123,
  content: {} as any,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editStory({ caption: "Response" });
});
```
...
| `messageId` | `this.messageIds?.id` | Required if inline_message_id is not specified. Identifier of the message to edit. |
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageText({
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editMessageText({ inlineMessageId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageCaption({
  inlineMessageId: "example text",
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editMessageCaption({ inlineMessageId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageMedia({
  media: {} as any,
  inlineMessageId: "example text",
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editMessageMedia({ inlineMessageId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editMessageLiveLocation({ inlineMessageId: "Response" });
});
```
...
| `businessConnectionId` | `this.businessConnectionId` | Unique identifier of the business connection on behalf of which the message will be sent |
| `chatId` | `this.chat?.id` | Unique identifier for the target chat or username of the target bot in the format @username |
| `messageId` | `this.messageIds?.id` | Unique identifier for the target message |
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageChecklist({
  checklist: {} as any,
  replyMarkup: {} as any,
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editMessageReplyMarkup({
  inlineMessageId: "example text",
  replyMarkup: {} as any,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editMessageReplyMarkup({ inlineMessageId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editEphemeralMessageText({
  receiverUserId: 123,
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editEphemeralMessageText({ text: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editEphemeralMessageMedia({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editEphemeralMessageCaption({
  receiverUserId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.editEphemeralMessageCaption({ caption: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendSticker({
  sticker: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendRichMessage({
  richMessage: {} as any,
  messageThreadId: 123,
});
```
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendRichMessage({ messageEffectId: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendInvoice({
  title: "example text",
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendInvoice({ title: "Response" });
});
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const businessmessagesdeleted = new BusinessMessagesDeleted(rawData, bot);
await businessmessagesdeleted.sendGame({
...
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  // Auto-fills parameters from the businessmessagesdeleted instance
  await businessmessagesdeleted.sendGame({ gameShortName: "Response" });
});
```
...
You can listen for BusinessMessagesDeleted events using:

```typescript
// Type-specific handler
bot.onBusinessMessagesDeleted(async (businessmessagesdeleted: BusinessMessagesDeleted) => {
  console.log('Received:', businessmessagesdeleted);
  // Use fluent methods
  await businessmessagesdeleted.sendMessage(...);
});

// Generic handler
bot.on('businessmessagesdeleted', async (data: BusinessMessagesDeleted) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#BusinessMessagesDeleted).
