...
```typescript
const userprofilephotos = new UserProfilePhotos(rawData, bot);
await userprofilephotos.sendChatAction(123, 'example text');
```
...
```typescript
const userprofilephotos = new UserProfilePhotos(rawData, bot);
await userprofilephotos.getUserProfilePhotos(123, 123);
```
...
bot.onUserProfilePhotos(async (userprofilephotos: UserProfilePhotos) => {
  // Auto-fills parameters from the userprofilephotos instance
  await userprofilephotos.sendGame({ gameShortName: 'Response' });
});
```
