...
| cover | `string` | No | Optional. Cover for the video in the message. Pass a file\_id to send a file that exists on the Telegram servers \(recommended\), pass an HTTP URL for Telegram to get a file from the Internet, or pass “attach://&lt;file\_attach\_name&gt;” to upload a new one using multipart/form-data under &lt;file\_attach\_name&gt; name. More information on Sending Files » |
| startTimestamp | `number` | No | Optional. Start timestamp for the video in the message |
| width | `number` | No | Optional. Video width |
| height | `number` | No | Optional. Video height |
| duration | `number` | No | Optional. Video duration in seconds |
| supportsStreaming | `boolean` | No | Optional. Pass True if the uploaded video is suitable for streaming |

## Fluent Methods

The `InputPaidMediaVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendContact({
  chatId: 123,
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendPoll({
  chatId: 123,
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendChecklist({
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputpaidmediavideo = new InputPaidMediaVideo(rawData, bot);
await inputpaidmediavideo.sendGame({
  chatId: 123,
...
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  // Auto-fills parameters from the inputpaidmediavideo instance
  await inputpaidmediavideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputPaidMediaVideo events using:

```typescript
// Type-specific handler
bot.onInputPaidMediaVideo(async (inputpaidmediavideo: InputPaidMediaVideo) => {
  console.log('Received:', inputpaidmediavideo);
  // Use fluent methods
  await inputpaidmediavideo.sendMessage(...);
});

// Generic handler
bot.on('inputpaidmediavideo', async (data: InputPaidMediaVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputPaidMediaVideo).
