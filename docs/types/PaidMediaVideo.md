...
| :--- | :--- | :---: | :--- |
| type | `string` | Yes | Type of the paid media, always “video” |
| video | `Video` | Yes | The video |

## Fluent Methods

The `PaidMediaVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendContact({
  chatId: 123,
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendContact({ chatId: "Response" });
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendChecklist({
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const paidmediavideo = new PaidMediaVideo(rawData, bot);
await paidmediavideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendInvoice({ chatId: "Response" });
});
```
...
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  // Auto-fills parameters from the paidmediavideo instance
  await paidmediavideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for PaidMediaVideo events using:

```typescript
// Type-specific handler
bot.onPaidMediaVideo(async (paidmediavideo: PaidMediaVideo) => {
  console.log('Received:', paidmediavideo);
  // Use fluent methods
  await paidmediavideo.sendMessage(...);
});

// Generic handler
bot.on('paidmediavideo', async (data: PaidMediaVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#PaidMediaVideo).
