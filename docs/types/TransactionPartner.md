...
You can listen for TransactionPartner events using:

```typescript
// Type-specific handler
bot.onTransactionPartner(async (transactionpartner: TransactionPartner) => {
  console.log('Received:', transactionpartner);
});

// Generic handler
bot.on('transactionpartner', async (data: TransactionPartner) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#TransactionPartner).
