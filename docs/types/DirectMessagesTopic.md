...
| :--- | :--- | :---: | :--- |
| topicId | `number` | Yes | Unique identifier of the topic. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier. |
| user | `User` | No | Optional. Information about the user that created the topic. Currently, it is always present. |

## Fluent Methods

The `DirectMessagesTopic` class has the following fluent methods that automatically inject contextual parameters:
...
| :--- | :--- | :--- |
| `directMessagesTopicId` | `this.id` | Identifier of the direct messages topic to which the message will be sent; required if the message is sent to a direct messages chat |
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendMessage({ chatId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `directMessagesTopicId` | `this.id` | Identifier of the direct messages topic to which the message will be forwarded; required if the message is forwarded to a direct messages chat |
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.forwardMessage({ chatId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `directMessagesTopicId` | `this.id` | Identifier of the direct messages topic to which the messages will be forwarded; required if the messages are forwarded to a direct messages chat |
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendPaidMedia({ chatId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `directMessagesTopicId` | `this.id` | Identifier of the direct messages topic to which the messages will be sent; required if the messages are sent to a direct messages chat |
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendContact({
  chatId: 123,
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendContact({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendPoll({
  chatId: 123,
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendChecklist({
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendDice({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editChatInviteLink({
  chatId: 123,
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editForumTopic(
  123,
  123,
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendGift({
  giftId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendGift({ giftId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editEphemeralMessageText({
  chatId: 123,
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editEphemeralMessageMedia({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editEphemeralMessageCaption({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.editUserStarSubscription(
  "example text",
  true,
);
```
...
```typescript
const directmessagestopic = new DirectMessagesTopic(rawData, bot);
await directmessagestopic.sendGame({
  chatId: 123,
...
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  // Auto-fills parameters from the directmessagestopic instance
  await directmessagestopic.sendGame({ chatId: "Response" });
});
```
...
You can listen for DirectMessagesTopic events using:

```typescript
// Type-specific handler
bot.onDirectMessagesTopic(async (directmessagestopic: DirectMessagesTopic) => {
  console.log('Received:', directmessagestopic);
  // Use fluent methods
  await directmessagestopic.sendMessage(...);
});

// Generic handler
bot.on('directmessagestopic', async (data: DirectMessagesTopic) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#DirectMessagesTopic).
