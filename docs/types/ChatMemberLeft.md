...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getManagedBotToken();
```
...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getManagedBotAccessSettings();
```
...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getUserGifts({
...
bot.onChatMemberLeft(async (chatmemberleft: ChatMemberLeft) => {
  // Auto-fills parameters from the chatmemberleft instance
  await chatmemberleft.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatmemberleft = new ChatMemberLeft(rawData, bot);
await chatmemberleft.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatMemberLeft(async (chatmemberleft: ChatMemberLeft) => {
  // Auto-fills parameters from the chatmemberleft instance
  await chatmemberleft.getChatGifts({ offset: "Response" });
});
```
