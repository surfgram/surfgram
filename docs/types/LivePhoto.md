...
| :--- | :--- | :---: | :--- |
| photo | `PhotoSize[]` | No | Optional. Available sizes of the corresponding static photo |
| fileId | `string` | Yes | Identifier for the video file which can be used to download or reuse the file |
| fileUniqueId | `string` | Yes | Unique identifier for the video file which is supposed to be the same over time and for different bots. Can't be used to download or reuse the file. |
...
The `LivePhoto` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendMessage({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendPhoto({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendAudio({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendDocument({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendVideo({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendAnimation({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendVoice({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendLocation({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendVenue({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendContact({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendPoll({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendDice({ chatId: "Response" });
});
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendGift({ giftId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const livephoto = new LivePhoto(rawData, bot);
await livephoto.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendInvoice({ chatId: "Response" });
});
```
...
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  // Auto-fills parameters from the livephoto instance
  await livephoto.sendGame({ chatId: "Response" });
});
```
...
You can listen for LivePhoto events using:

```typescript
// Type-specific handler
bot.onLivePhoto(async (livephoto: LivePhoto) => {
  console.log('Received:', livephoto);
  // Use fluent methods
  await livephoto.sendMessage(...);
});

// Generic handler
bot.on('livephoto', async (data: LivePhoto) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#LivePhoto).
