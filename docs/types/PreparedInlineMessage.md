...
| expirationDate | `number` | Yes | Expiration date of the prepared message, in Unix time. Expired prepared messages can no longer be used. |

## Fluent Methods

The `PreparedInlineMessage` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendContact({
  chatId: 123,
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendContact({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendPoll({
  chatId: 123,
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendChecklist({
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendDice({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editChatInviteLink({
  chatId: 123,
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editForumTopic(
  123,
  123,
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendGift({ giftId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.savePreparedInlineMessage({
  userId: 123,
  result: {} as any,
});
```
...
| `inlineMessageId` | `this?.id` | Required if chat_id and message_id are not specified. Identifier of the inline message. |
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const preparedinlinemessage = new PreparedInlineMessage(rawData, bot);
await preparedinlinemessage.sendGame({
  chatId: 123,
...
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  // Auto-fills parameters from the preparedinlinemessage instance
  await preparedinlinemessage.sendGame({ chatId: "Response" });
});
```
...
You can listen for PreparedInlineMessage events using:

```typescript
// Type-specific handler
bot.onPreparedInlineMessage(async (preparedinlinemessage: PreparedInlineMessage) => {
  console.log('Received:', preparedinlinemessage);
  // Use fluent methods
  await preparedinlinemessage.sendMessage(...);
});

// Generic handler
bot.on('preparedinlinemessage', async (data: PreparedInlineMessage) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#PreparedInlineMessage).
