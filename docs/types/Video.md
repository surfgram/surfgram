...
The `Video` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const video = new Video(rawData, bot);
await video.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendMessage({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendPhoto({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendAudio({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendDocument({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendVideo({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendAnimation({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendVoice({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendPaidMedia({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendLocation({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendVenue({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendContact({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendPoll({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendDice({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const video = new Video(rawData, bot);
await video.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendGift({ giftId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendSticker({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const video = new Video(rawData, bot);
await video.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendInvoice({ chatId: "Response" });
});
```
...
bot.onVideo(async (video: Video) => {
  // Auto-fills parameters from the video instance
  await video.sendGame({ chatId: "Response" });
});
```
...
You can listen for Video events using:
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#Video).
