...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getManagedBotToken(
  123,
);
```
...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getUserGifts({
  userId: 123,
...
bot.onBotCommandScopeChat(async (botcommandscopechat: BotCommandScopeChat) => {
  // Auto-fills parameters from the botcommandscopechat instance
  await botcommandscopechat.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const botcommandscopechat = new BotCommandScopeChat(rawData, bot);
await botcommandscopechat.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onBotCommandScopeChat(async (botcommandscopechat: BotCommandScopeChat) => {
  // Auto-fills parameters from the botcommandscopechat instance
  await botcommandscopechat.getChatGifts({ offset: "Response" });
});
```
