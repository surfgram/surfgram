...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getManagedBotToken();
```
...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getManagedBotAccessSettings();
```
...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getUserGifts({
...
bot.onChatBoostSourcePremium(async (chatboostsourcepremium: ChatBoostSourcePremium) => {
  // Auto-fills parameters from the chatboostsourcepremium instance
  await chatboostsourcepremium.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostsourcepremium = new ChatBoostSourcePremium(rawData, bot);
await chatboostsourcepremium.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostSourcePremium(async (chatboostsourcepremium: ChatBoostSourcePremium) => {
  // Auto-fills parameters from the chatboostsourcepremium instance
  await chatboostsourcepremium.getChatGifts({ offset: "Response" });
});
```
