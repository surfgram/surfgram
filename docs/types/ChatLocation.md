...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getManagedBotToken(
  123,
);
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUserGifts({
  userId: 123,
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getChatGifts({ offset: "Response" });
});
```
