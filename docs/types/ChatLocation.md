...
| :------- | :--------- | :------: | :--------------------------------------------------------------------------- |
| location | `Location` |   Yes    | The location to which the supergroup is connected. Can't be a live location. |
| address  | `string`   |   Yes    | Location address; 1-64 characters, as defined by the chat owner              |

## Fluent Methods

The `ChatLocation` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getUpdates({});
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getWebhookInfo({
  url: 'example text',
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getWebhookInfo({ url: 'Response' });
});
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMe({ text: 'Response' });
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUserProfilePhotos(123, 123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getUserProfilePhotos();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getFile('example text');
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getChatAdministrators();
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getChatMemberCount();
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getChatMember(123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getChatMember();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getForumTopicIconStickers('example text', 123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getForumTopicIconStickers();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getUserChatBoosts(123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getUserChatBoosts();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getBusinessConnection('example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getBusinessConnection();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyCommands({} as any, 'example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMyCommands();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyName('example text');
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyDescription('example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMyDescription();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyShortDescription('example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMyShortDescription();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getChatMenuButton();
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyDefaultAdministratorRights(true);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMyDefaultAdministratorRights();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getAvailableGifts({
  giftId: 'example text',
  userId: 123,
});
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getAvailableGifts({ giftId: 'Response' });
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getBusinessAccountStarBalance('example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getBusinessAccountStarBalance();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getBusinessAccountGifts({
  businessConnectionId: 'example text',
  excludeUnsaved: true,
});
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getBusinessAccountGifts({ businessConnectionId: 'Response' });
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getStickerSet('example text');
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getStickerSet();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getCustomEmojiStickers(['example text']);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getCustomEmojiStickers();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getMyStarBalance(123, 123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getMyStarBalance();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getStarTransactions(123, 123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getStarTransactions();
});
```
...
```typescript
const chatlocation = new ChatLocation(rawData, bot);
await chatlocation.getGameHighScores(123, 123);
```
...
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  // Auto-fills parameters from the chatlocation instance
  await chatlocation.getGameHighScores();
});
```
...
You can listen for ChatLocation events using:

```typescript
// Type-specific handler
bot.onChatLocation(async (chatlocation: ChatLocation) => {
  console.log('Received:', chatlocation);
  // Use fluent methods
  await chatlocation.getUpdates(...);
});

// Generic handler
bot.on('chatlocation', async (data: ChatLocation) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#ChatLocation).
