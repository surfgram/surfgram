...
| hasSpoiler | `boolean` | No | Optional. Pass True if the video needs to be covered with a spoiler animation |

## Fluent Methods

The `InputMediaVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendContact({
  chatId: 123,
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendPoll({
  chatId: 123,
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendChecklist({
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputmediavideo = new InputMediaVideo(rawData, bot);
await inputmediavideo.sendGame({
  chatId: 123,
...
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  // Auto-fills parameters from the inputmediavideo instance
  await inputmediavideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputMediaVideo events using:

```typescript
// Type-specific handler
bot.onInputMediaVideo(async (inputmediavideo: InputMediaVideo) => {
  console.log('Received:', inputmediavideo);
  // Use fluent methods
  await inputmediavideo.sendMessage(...);
});

// Generic handler
bot.on('inputmediavideo', async (data: InputMediaVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputMediaVideo).
