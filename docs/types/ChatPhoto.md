...
bot.onChatPhoto(async (chatphoto: ChatPhoto) => {
  // Auto-fills parameters from the chatphoto instance
  await chatphoto.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onChatPhoto(async (chatphoto: ChatPhoto) => {
  // Auto-fills parameters from the chatphoto instance
  await chatphoto.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.getManagedBotToken(
  123,
);
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.getManagedBotAccessSettings(
  123,
);
```
...
bot.onChatPhoto(async (chatphoto: ChatPhoto) => {
  // Auto-fills parameters from the chatphoto instance
  await chatphoto.getUserGifts({ offset: "Response" });
});
```
...
bot.onChatPhoto(async (chatphoto: ChatPhoto) => {
  // Auto-fills parameters from the chatphoto instance
  await chatphoto.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onChatPhoto(async (chatphoto: ChatPhoto) => {
  // Auto-fills parameters from the chatphoto instance
  await chatphoto.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const chatphoto = new ChatPhoto(rawData, bot);
await chatphoto.sendRichMessageDraft(
  123,
  {} as any,
);
```
