...
| :--- | :--- | :---: | :--- |
| community | `Community` | Yes | The new community to which the chat belongs |

## Fluent Methods

The `CommunityChatAdded` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getUpdates({});
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getWebhookInfo({
  url: "example text",
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getWebhookInfo({ url: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMe({
  text: "example text",
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getMe({ text: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getFile(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChat();
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChatAdministrators(
  true,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChatMemberCount();
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChatMember(
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUserChatBoosts(
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getBusinessConnection(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getManagedBotToken(
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyName(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyDescription(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyShortDescription(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChatMenuButton();
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyDefaultAdministratorRights(
  true,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getAvailableGifts({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getUserGifts({
  userId: 123,
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  // Auto-fills parameters from the communitychatadded instance
  await communitychatadded.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getStickerSet(
  "example text",
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getStarTransactions(
  123,
  123,
);
```
...
```typescript
const communitychatadded = new CommunityChatAdded(rawData, bot);
await communitychatadded.getGameHighScores(
  123,
  123,
);
```
...
You can listen for CommunityChatAdded events using:

```typescript
// Type-specific handler
bot.onCommunityChatAdded(async (communitychatadded: CommunityChatAdded) => {
  console.log('Received:', communitychatadded);
  // Use fluent methods
  await communitychatadded.getUpdates(...);
});

// Generic handler
bot.on('communitychatadded', async (data: CommunityChatAdded) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#CommunityChatAdded).
