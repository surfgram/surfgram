...
| senderUser | `User` | Yes | User that sent the message originally |

## Fluent Methods

The `MessageOriginUser` class has the following fluent methods that automatically inject contextual parameters:
...
| :--- | :--- | :--- |
| `receiverUserId` | `this.senderUser?.id` | For outgoing ephemeral messages, unique identifier of the user who will receive the message; for group and supergroup chats only. It is not guaranteed that the user will receive the message, especially if they are offline. See ephemeral message sending for more details. |
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendContact({
  chatId: 123,
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendContact({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendPoll({
  chatId: 123,
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendChecklist({
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendDice({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editChatInviteLink({
  chatId: 123,
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editForumTopic(
  123,
  123,
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editGeneralForumTopic(
  123,
  "example text",
);
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.senderUser?.id` | Required if chat_id is not specified. Unique identifier of the target user who will receive the gift. |
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendGift({
  giftId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendGift({ giftId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `receiverUserId` | `this.senderUser?.id` | Identifier of the user who received the message |
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editEphemeralMessageText({
  chatId: 123,
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editEphemeralMessageMedia({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editEphemeralMessageCaption({
  chatId: 123,
  ephemeralMessageId: 123,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendInvoice({ chatId: "Response" });
});
```
...
| Parameter | Source | Description |
| :--- | :--- | :--- |
| `userId` | `this.senderUser?.id` | Identifier of the user whose subscription will be edited |
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.editUserStarSubscription(
  "example text",
  true,
);
```
...
```typescript
const messageoriginuser = new MessageOriginUser(rawData, bot);
await messageoriginuser.sendGame({
  chatId: 123,
...
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  // Auto-fills parameters from the messageoriginuser instance
  await messageoriginuser.sendGame({ chatId: "Response" });
});
```
...
You can listen for MessageOriginUser events using:

```typescript
// Type-specific handler
bot.onMessageOriginUser(async (messageoriginuser: MessageOriginUser) => {
  console.log('Received:', messageoriginuser);
  // Use fluent methods
  await messageoriginuser.sendMessage(...);
});

// Generic handler
bot.on('messageoriginuser', async (data: MessageOriginUser) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#MessageOriginUser).
