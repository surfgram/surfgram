...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getManagedBotToken();
```
...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getManagedBotAccessSettings();
```
...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getUserGifts({
...
bot.onChatBoostSource(async (chatboostsource: ChatBoostSource) => {
  // Auto-fills parameters from the chatboostsource instance
  await chatboostsource.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatboostsource = new ChatBoostSource(rawData, bot);
await chatboostsource.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatBoostSource(async (chatboostsource: ChatBoostSource) => {
  // Auto-fills parameters from the chatboostsource instance
  await chatboostsource.getChatGifts({ offset: "Response" });
});
```
