...
The `PhotoSize` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendMessage({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendPhoto({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendLivePhoto({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendAudio({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendDocument({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendVideo({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendAnimation({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendVoice({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendVideoNote({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendMediaGroup({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendLocation({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendVenue({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendContact({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendPoll({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendDice({ chatId: "Response" });
});
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendGift({ giftId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const photosize = new PhotoSize(rawData, bot);
await photosize.sendRichMessageDraft(
  123,
  123,
);
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendInvoice({ chatId: "Response" });
});
```
...
bot.onPhotoSize(async (photosize: PhotoSize) => {
  // Auto-fills parameters from the photosize instance
  await photosize.sendGame({ chatId: "Response" });
});
```
...
You can listen for PhotoSize events using:

```typescript
// Type-specific handler
bot.onPhotoSize(async (photosize: PhotoSize) => {
  console.log('Received:', photosize);
  // Use fluent methods
  await photosize.sendMessage(...);
});

// Generic handler
bot.on('photosize', async (data: PhotoSize) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#PhotoSize).
