...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getUserPersonalChatMessages(
  123,
);
```
...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getManagedBotToken();
```
...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getManagedBotAccessSettings();
```
...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getUserGifts({
...
bot.onChatMemberOwner(async (chatmemberowner: ChatMemberOwner) => {
  // Auto-fills parameters from the chatmemberowner instance
  await chatmemberowner.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const chatmemberowner = new ChatMemberOwner(rawData, bot);
await chatmemberowner.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onChatMemberOwner(async (chatmemberowner: ChatMemberOwner) => {
  // Auto-fills parameters from the chatmemberowner instance
  await chatmemberowner.getChatGifts({ offset: "Response" });
});
```
