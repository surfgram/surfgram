...
| :--- | :--- | :---: | :--- |
| startDate | `number` | Yes | Point in time \(Unix timestamp\) when the video chat is supposed to be started by a chat administrator |

## Fluent Methods

The `VideoChatScheduled` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUpdates({
  offset: 123,
  limit: 123,
});
```
...
```typescript
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getUpdates({});
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getWebhookInfo({
  url: "example text",
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getWebhookInfo({ url: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMe({
  text: "example text",
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getMe({ text: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendMessage({
  text: "example text",
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendMessage({ text: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendPhoto({
  photo: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendPhoto({ photo: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendLivePhoto({
  livePhoto: {} as any,
  photo: {} as any,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendLivePhoto({ livePhoto: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendAudio({
  audio: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendAudio({ audio: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendDocument({
  document: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendDocument({ document: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendVideo({
  video: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendVideo({ video: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendAnimation({
  animation: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendAnimation({ animation: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendVoice({
  voice: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendVoice({ voice: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendVideoNote({
  videoNote: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendVideoNote({ videoNote: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendPaidMedia({
  starCount: 123,
  media: [{} as any],
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendPaidMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendMediaGroup({
  media: [{} as any],
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendMediaGroup({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendVenue({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendVenue({ title: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendContact({
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendContact({ phoneNumber: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendPoll({
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendPoll({ question: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendChecklist({
  businessConnectionId: "example text",
  checklist: {} as any,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendDice({
  businessConnectionId: "example text",
  messageThreadId: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendDice({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendMessageDraft({
  draftId: 123,
  messageThreadId: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendChatAction(
  "example text",
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUserProfilePhotos(
  123,
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUserProfileAudios(
  123,
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getFile(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChat();
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChatAdministrators(
  true,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChatMemberCount();
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChatMember(
  123,
);
```
...
| Parameter | Type | Required | Description |
| :--- | :--- | :---: | :--- |
| `userId` | `number` | Yes | Unique identifier for the target user |
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUserPersonalChatMessages(
  123,
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getForumTopicIconStickers(
  "example text",
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUserChatBoosts(
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getBusinessConnection(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getManagedBotToken(
  123,
);
```
...
| Parameter | Type | Required | Description |
| :--- | :--- | :---: | :--- |
| `userId` | `number` | Yes | User identifier of the managed bot whose access settings will be returned |
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getManagedBotAccessSettings(
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyCommands(
  {} as any,
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyName(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyDescription(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyShortDescription(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChatMenuButton();
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyDefaultAdministratorRights(
  true,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getAvailableGifts({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getAvailableGifts({ giftId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendGift({ giftId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getBusinessAccountStarBalance(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getBusinessAccountGifts({
  businessConnectionId: "example text",
  excludeUnsaved: true,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getBusinessAccountGifts({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getUserGifts({
  userId: 123,
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getUserGifts({ offset: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getChatGifts({
  excludeUnsaved: true,
  excludeSaved: true,
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.getChatGifts({ offset: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendSticker({
  sticker: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendSticker({ sticker: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getStickerSet(
  "example text",
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getCustomEmojiStickers(
  ["example text"],
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendRichMessage({
  richMessage: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendRichMessage({ businessConnectionId: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendRichMessageDraft(
  123,
  {} as any,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendInvoice({
  title: "example text",
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendInvoice({ title: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getMyStarBalance(
  123,
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getStarTransactions(
  123,
  123,
);
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.sendGame({
...
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  // Auto-fills parameters from the videochatscheduled instance
  await videochatscheduled.sendGame({ gameShortName: "Response" });
});
```
...
```typescript
const videochatscheduled = new VideoChatScheduled(rawData, bot);
await videochatscheduled.getGameHighScores(
  123,
  123,
);
```
...
You can listen for VideoChatScheduled events using:

```typescript
// Type-specific handler
bot.onVideoChatScheduled(async (videochatscheduled: VideoChatScheduled) => {
  console.log('Received:', videochatscheduled);
  // Use fluent methods
  await videochatscheduled.getUpdates(...);
});

// Generic handler
bot.on('videochatscheduled', async (data: VideoChatScheduled) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#VideoChatScheduled).
