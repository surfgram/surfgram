...
| media | `InputMediaAnimation` \| `InputMediaAudio` \| `InputMediaPhoto` \| `InputMediaVideo` \| `InputMediaVoiceNote` | Yes | The media to be sent. Everything except the media itself and its properties is ignored. |

## Fluent Methods

The `InputRichMessageMedia` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.forwardMessage({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.forwardMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.forwardMessages({
  chatId: 123,
  fromChatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.forwardMessages({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendContact({
  chatId: 123,
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendContact({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendPoll({
  chatId: 123,
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendChecklist({
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendDice({
  chatId: 123,
  businessConnectionId: "example text",
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendDice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editChatInviteLink({
  chatId: 123,
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editChatInviteLink({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editChatSubscriptionInviteLink(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editForumTopic(
  123,
  123,
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editGeneralForumTopic(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendGift({
  giftId: "example text",
  userId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendGift({ giftId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editStory({
  businessConnectionId: "example text",
  storyId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editStory({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageText({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageText({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageCaption({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageCaption({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageMedia({
  media: {} as any,
  businessConnectionId: "example text",
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageMedia({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageLiveLocation({
  latitude: 123,
  longitude: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageLiveLocation({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageChecklist({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageChecklist({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editMessageReplyMarkup({
  businessConnectionId: "example text",
  chatId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editMessageReplyMarkup({ businessConnectionId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editEphemeralMessageText({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editEphemeralMessageText({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editEphemeralMessageMedia({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editEphemeralMessageMedia({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editEphemeralMessageCaption({
  chatId: 123,
  receiverUserId: 123,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.editEphemeralMessageCaption({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editEphemeralMessageReplyMarkup(
  123,
  123,
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendInvoice({ chatId: "Response" });
});
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.editUserStarSubscription(
  123,
  "example text",
);
```
...
```typescript
const inputrichmessagemedia = new InputRichMessageMedia(rawData, bot);
await inputrichmessagemedia.sendGame({
  chatId: 123,
...
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  // Auto-fills parameters from the inputrichmessagemedia instance
  await inputrichmessagemedia.sendGame({ chatId: "Response" });
});
```
...
You can listen for InputRichMessageMedia events using:

```typescript
// Type-specific handler
bot.onInputRichMessageMedia(async (inputrichmessagemedia: InputRichMessageMedia) => {
  console.log('Received:', inputrichmessagemedia);
  // Use fluent methods
  await inputrichmessagemedia.sendMessage(...);
});

// Generic handler
bot.on('inputrichmessagemedia', async (data: InputRichMessageMedia) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#InputRichMessageMedia).
