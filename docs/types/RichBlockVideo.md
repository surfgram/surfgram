...
The `RichBlockVideo` class has the following fluent methods that automatically inject contextual parameters:
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendMessage({
  chatId: 123,
  text: "example text",
});
```
...
```typescript
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendMessage({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendPhoto({
  chatId: 123,
  photo: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendPhoto({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendLivePhoto({
  chatId: 123,
  livePhoto: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendLivePhoto({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendAudio({
  chatId: 123,
  audio: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendAudio({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendDocument({
  chatId: 123,
  document: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendDocument({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendVideo({
  chatId: 123,
  video: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendVideo({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendAnimation({
  chatId: 123,
  animation: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendAnimation({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendVoice({
  chatId: 123,
  voice: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendVoice({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendVideoNote({
  chatId: 123,
  videoNote: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendVideoNote({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendPaidMedia({
  chatId: 123,
  starCount: 123,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendPaidMedia({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendMediaGroup({
  chatId: 123,
  media: [{} as any],
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendMediaGroup({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendLocation({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendLocation({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendVenue({
  chatId: 123,
  latitude: 123,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendVenue({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendContact({
  chatId: 123,
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendContact({ chatId: "Response" });
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendPoll({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendChecklist({
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendChecklist({ businessConnectionId: "Response" });
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendDice({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendMessageDraft({
  chatId: 123,
  draftId: 123,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendMessageDraft({ text: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendChatAction(
  123,
  "example text",
);
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendChatJoinRequestWebApp(
  "example text",
  "example text",
);
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendGift({ giftId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendSticker({
  chatId: 123,
  sticker: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendSticker({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendRichMessage({
  chatId: 123,
  richMessage: {} as any,
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendRichMessage({ chatId: "Response" });
});
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendRichMessageDraft(
  123,
  123,
);
```
...
```typescript
const richblockvideo = new RichBlockVideo(rawData, bot);
await richblockvideo.sendInvoice({
  chatId: 123,
  title: "example text",
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendInvoice({ chatId: "Response" });
});
```
...
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  // Auto-fills parameters from the richblockvideo instance
  await richblockvideo.sendGame({ chatId: "Response" });
});
```
...
You can listen for RichBlockVideo events using:

```typescript
// Type-specific handler
bot.onRichBlockVideo(async (richblockvideo: RichBlockVideo) => {
  console.log('Received:', richblockvideo);
  // Use fluent methods
  await richblockvideo.sendMessage(...);
});

// Generic handler
bot.on('richblockvideo', async (data: RichBlockVideo) => {
  // Same as above
});
```
...
For more details, see the official [Telegram Bot API documentation](https://core.telegram.org/bots/api#RichBlockVideo).
