...
describe("Bot", () => {
	...
	const createBot = () => {
		bot = new Bot(mockToken);
		(bot as any).deleteWebhook = jest
			.fn<() => Promise<void>>()
			.mockResolvedValue(undefined);
		(bot as any).setWebhook = jest
			.fn<() => Promise<void>>()
			.mockResolvedValue(undefined);
		return bot;
	};
	...
	describe("callApi", () => {
		...
		test("should throw error when API returns not ok", async () => {
			...
			await expect(bot.callApi(mockMethod, mockParams)).rejects.toThrow(
				"Not Found",
			);
		});
	...
	describe("use", () => {
		...
		test("should use plugin", () => {
			...
			expect(mockPluginManagerInstance.use).toHaveBeenCalledWith(
				mockPlugin,
				options,
			);
			expect(result).toBe(bot);
