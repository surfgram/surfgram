...
describe("ShippingAddress", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.streetLine1 ?? instance.raw?.street_line1).toEqual(
			"example text",
		);
		expect(instance.streetLine2 ?? instance.raw?.street_line2).toEqual(
			"example text",
		);
		expect(instance.postCode ?? instance.raw?.post_code).toEqual(
			"example text",
		);
	});
});
