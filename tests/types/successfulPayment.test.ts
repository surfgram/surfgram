...
describe('SuccessfulPayment', () => {
  ...
  it('should correctly instantiate from raw data', () => {
    ...
    expect(instance.subscriptionExpirationDate ?? instance.raw?.subscription_expiration_date).toEqual(123);
    expect(instance.isRecurring ?? instance.raw?.is_recurring).toEqual(true);
    expect(instance.isFirstRecurring ?? instance.raw?.is_first_recurring).toEqual(true);
    expect(instance.shippingOptionId ?? instance.raw?.shipping_option_id).toEqual("example text");
    expect(instance.orderInfo ?? instance.raw?.order_info).toEqual({} as any);
    expect(instance.telegramPaymentChargeId ?? instance.raw?.telegram_payment_charge_id).toEqual("example text");
    expect(instance.providerPaymentChargeId ?? instance.raw?.provider_payment_charge_id).toEqual("example text");
  });
});