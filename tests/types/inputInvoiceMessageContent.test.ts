...
describe('InputInvoiceMessageContent', () => {
  ...
  it('should correctly instantiate from raw data', () => {
    ...
    expect(instance.providerToken ?? instance.raw?.provider_token).toEqual("example text");
    ...
    expect(instance.suggestedTipAmounts ?? instance.raw?.suggested_tip_amounts).toEqual([123]);
    expect(instance.providerData ?? instance.raw?.provider_data).toEqual("example text");
    expect(instance.photoUrl ?? instance.raw?.photo_url).toEqual("example text");
    ...
    expect(instance.needPhoneNumber ?? instance.raw?.need_phone_number).toEqual(true);
    expect(instance.needEmail ?? instance.raw?.need_email).toEqual(true);
    expect(instance.needShippingAddress ?? instance.raw?.need_shipping_address).toEqual(true);
    expect(instance.sendPhoneNumberToProvider ?? instance.raw?.send_phone_number_to_provider).toEqual(true);
    expect(instance.sendEmailToProvider ?? instance.raw?.send_email_to_provider).toEqual(true);
