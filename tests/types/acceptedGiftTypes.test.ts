...
describe("AcceptedGiftTypes", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.unlimitedGifts ?? instance.raw?.unlimited_gifts).toEqual(
			true,
		);
		...
		expect(
			instance.premiumSubscription ?? instance.raw?.premium_subscription,
		).toEqual(true);
		expect(
			instance.giftsFromChannels ?? instance.raw?.gifts_from_channels,
		).toEqual(true);
	});
});
