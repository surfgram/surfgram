...
describe("InlineKeyboardButton", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.callbackData ?? instance.raw?.callback_data).toEqual(
			"example text",
		);
		expect(instance.webApp ?? instance.raw?.web_app).toEqual({} as any);
		expect(instance.loginUrl ?? instance.raw?.login_url).toEqual({} as any);
		expect(
			instance.switchInlineQuery ?? instance.raw?.switch_inline_query,
		).toEqual("example text");
		expect(
			instance.switchInlineQueryCurrentChat ??
				instance.raw?.switch_inline_query_current_chat,
		).toEqual("example text");
		expect(
			instance.switchInlineQueryChosenChat ??
				instance.raw?.switch_inline_query_chosen_chat,
		).toEqual({} as any);
		expect(instance.copyText ?? instance.raw?.copy_text).toEqual({} as any);
		expect(instance.callbackGame ?? instance.raw?.callback_game).toEqual(
			{} as any,
		);
