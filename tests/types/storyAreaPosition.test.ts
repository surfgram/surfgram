...
describe("StoryAreaPosition", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.widthPercentage ?? instance.raw?.width_percentage).toEqual(
			123,
		);
		expect(
			instance.heightPercentage ?? instance.raw?.height_percentage,
		).toEqual(123);
		expect(instance.rotationAngle ?? instance.raw?.rotation_angle).toEqual(123);
		expect(
			instance.cornerRadiusPercentage ?? instance.raw?.corner_radius_percentage,
		).toEqual(123);
	});
});
