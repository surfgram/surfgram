...
describe("LinkPreviewOptions", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(
			instance.preferSmallMedia ?? instance.raw?.prefer_small_media,
		).toEqual(true);
		expect(
			instance.preferLargeMedia ?? instance.raw?.prefer_large_media,
		).toEqual(true);
		expect(instance.showAboveText ?? instance.raw?.show_above_text).toEqual(
			true,
		);
	});
});
