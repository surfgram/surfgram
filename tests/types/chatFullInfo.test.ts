...
describe("ChatFullInfo", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(
			instance.firstProfileAudio ?? instance.raw?.first_profile_audio,
		).toEqual({} as any);
		expect(
			instance.uniqueGiftColors ?? instance.raw?.unique_gift_colors,
		).toEqual({} as any);
		expect(
			instance.paidMessageStarCount ?? instance.raw?.paid_message_star_count,
		).toEqual(123);
	});
});
