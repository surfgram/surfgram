...
describe("Message", () => {
	...
	it("should correctly instantiate from raw data", () => {
		const raw = {
			...
			chat_owner_left: {} as any,
			chat_owner_changed: {} as any,
		...
		expect(
			instance.directMessagesTopic ?? instance.raw?.direct_messages_topic,
		).toEqual({} as any);
		expect(instance.from ?? instance.raw?.from).toEqual({} as any);
		expect(instance.senderChat ?? instance.raw?.sender_chat).toEqual({} as any);
		expect(
			instance.senderBoostCount ?? instance.raw?.sender_boost_count,
		).toEqual(123);
		expect(
			instance.senderBusinessBot ?? instance.raw?.sender_business_bot,
		).toEqual({} as any);
		expect(instance.senderTag ?? instance.raw?.sender_tag).toEqual(
			"example text",
		);
		expect(instance.date ?? instance.raw?.date).toEqual(123);
		expect(instance.guestQueryId ?? instance.raw?.guest_query_id).toEqual(
			"example text",
		);
		expect(
		...
		expect(instance.forwardOrigin ?? instance.raw?.forward_origin).toEqual(
			{} as any,
		);
		expect(instance.isTopicMessage ?? instance.raw?.is_topic_message).toEqual(
			true,
		);
		expect(
			instance.isAutomaticForward ?? instance.raw?.is_automatic_forward,
		).toEqual(true);
		expect(instance.replyToMessage ?? instance.raw?.reply_to_message).toEqual(
			{} as any,
		);
		expect(instance.externalReply ?? instance.raw?.external_reply).toEqual(
			{} as any,
		);
		expect(instance.quote ?? instance.raw?.quote).toEqual({} as any);
		expect(instance.replyToStory ?? instance.raw?.reply_to_story).toEqual(
			{} as any,
		);
		expect(
			instance.replyToChecklistTaskId ??
				instance.raw?.reply_to_checklist_task_id,
		).toEqual(123);
		expect(
			instance.replyToPollOptionId ?? instance.raw?.reply_to_poll_option_id,
		).toEqual("example text");
		expect(instance.viaBot ?? instance.raw?.via_bot).toEqual({} as any);
		expect(
			instance.guestBotCallerUser ?? instance.raw?.guest_bot_caller_user,
		).toEqual({} as any);
		expect(
			instance.guestBotCallerChat ?? instance.raw?.guest_bot_caller_chat,
		).toEqual({} as any);
		...
		expect(instance.isFromOffline ?? instance.raw?.is_from_offline).toEqual(
			true,
		);
		expect(instance.isPaidPost ?? instance.raw?.is_paid_post).toEqual(true);
		expect(instance.mediaGroupId ?? instance.raw?.media_group_id).toEqual(
			"example text",
		);
		expect(instance.authorSignature ?? instance.raw?.author_signature).toEqual(
			"example text",
		);
		expect(instance.paidStarCount ?? instance.raw?.paid_star_count).toEqual(
			123,
		);
		...
		expect(
			instance.suggestedPostInfo ?? instance.raw?.suggested_post_info,
		).toEqual({} as any);
		expect(instance.effectId ?? instance.raw?.effect_id).toEqual(
			"example text",
		);
		...
		expect(instance.hasMediaSpoiler ?? instance.raw?.has_media_spoiler).toEqual(
			true,
		);
		...
		expect(instance.newChatMembers ?? instance.raw?.new_chat_members).toEqual([
			{} as any,
		]);
		expect(instance.leftChatMember ?? instance.raw?.left_chat_member).toEqual(
			{} as any,
		);
		expect(instance.chatOwnerLeft ?? instance.raw?.chat_owner_left).toEqual(
			{} as any,
		);
		expect(
			instance.chatOwnerChanged ?? instance.raw?.chat_owner_changed,
		).toEqual({} as any);
		expect(instance.newChatTitle ?? instance.raw?.new_chat_title).toEqual(
			"example text",
		);
		expect(instance.newChatPhoto ?? instance.raw?.new_chat_photo).toEqual([
			{} as any,
		]);
		expect(instance.deleteChatPhoto ?? instance.raw?.delete_chat_photo).toEqual(
			true,
		);
		expect(
			instance.groupChatCreated ?? instance.raw?.group_chat_created,
		).toEqual(true);
		expect(
			instance.supergroupChatCreated ?? instance.raw?.supergroup_chat_created,
		).toEqual(true);
		expect(
			instance.channelChatCreated ?? instance.raw?.channel_chat_created,
		).toEqual(true);
		expect(
			instance.messageAutoDeleteTimerChanged ??
				instance.raw?.message_auto_delete_timer_changed,
		).toEqual({} as any);
		expect(
			instance.migrateToChatId ?? instance.raw?.migrate_to_chat_id,
		).toEqual(123);
		expect(
			instance.migrateFromChatId ?? instance.raw?.migrate_from_chat_id,
		).toEqual(123);
		...
		expect(
			instance.successfulPayment ?? instance.raw?.successful_payment,
		).toEqual({} as any);
		expect(instance.refundedPayment ?? instance.raw?.refunded_payment).toEqual(
			{} as any,
		);
		expect(instance.usersShared ?? instance.raw?.users_shared).toEqual(
			{} as any,
		);
		...
		expect(instance.giftUpgradeSent ?? instance.raw?.gift_upgrade_sent).toEqual(
			{} as any,
		);
		expect(
			instance.connectedWebsite ?? instance.raw?.connected_website,
		).toEqual("example text");
		expect(
			instance.writeAccessAllowed ?? instance.raw?.write_access_allowed,
		).toEqual({} as any);
		expect(instance.passportData ?? instance.raw?.passport_data).toEqual(
			{} as any,
		);
		expect(
			instance.proximityAlertTriggered ??
				instance.raw?.proximity_alert_triggered,
		).toEqual({} as any);
		expect(instance.boostAdded ?? instance.raw?.boost_added).toEqual({} as any);
		expect(
			instance.chatBackgroundSet ?? instance.raw?.chat_background_set,
		).toEqual({} as any);
		expect(
			instance.checklistTasksDone ?? instance.raw?.checklist_tasks_done,
		).toEqual({} as any);
		expect(
			instance.checklistTasksAdded ?? instance.raw?.checklist_tasks_added,
		).toEqual({} as any);
		expect(
			instance.directMessagePriceChanged ??
				instance.raw?.direct_message_price_changed,
		).toEqual({} as any);
		expect(
			instance.forumTopicCreated ?? instance.raw?.forum_topic_created,
		).toEqual({} as any);
		expect(
			instance.forumTopicEdited ?? instance.raw?.forum_topic_edited,
		).toEqual({} as any);
		expect(
			instance.forumTopicClosed ?? instance.raw?.forum_topic_closed,
		).toEqual({} as any);
		expect(
			instance.forumTopicReopened ?? instance.raw?.forum_topic_reopened,
		).toEqual({} as any);
		expect(
			instance.generalForumTopicHidden ??
				instance.raw?.general_forum_topic_hidden,
		).toEqual({} as any);
		expect(
			instance.generalForumTopicUnhidden ??
				instance.raw?.general_forum_topic_unhidden,
		).toEqual({} as any);
		expect(instance.giveawayCreated ?? instance.raw?.giveaway_created).toEqual(
			{} as any,
		);
		expect(instance.giveaway ?? instance.raw?.giveaway).toEqual({} as any);
		expect(instance.giveawayWinners ?? instance.raw?.giveaway_winners).toEqual(
			{} as any,
		);
		expect(
			instance.giveawayCompleted ?? instance.raw?.giveaway_completed,
		).toEqual({} as any);
		expect(
			instance.managedBotCreated ?? instance.raw?.managed_bot_created,
		).toEqual({} as any);
		expect(
			instance.paidMessagePriceChanged ??
				instance.raw?.paid_message_price_changed,
		).toEqual({} as any);
		expect(instance.pollOptionAdded ?? instance.raw?.poll_option_added).toEqual(
			{} as any,
		);
		expect(
			instance.pollOptionDeleted ?? instance.raw?.poll_option_deleted,
		).toEqual({} as any);
		expect(
			instance.suggestedPostApproved ?? instance.raw?.suggested_post_approved,
		).toEqual({} as any);
		expect(
			instance.suggestedPostApprovalFailed ??
				instance.raw?.suggested_post_approval_failed,
		).toEqual({} as any);
		expect(
			instance.suggestedPostDeclined ?? instance.raw?.suggested_post_declined,
		).toEqual({} as any);
		expect(
			instance.suggestedPostPaid ?? instance.raw?.suggested_post_paid,
		).toEqual({} as any);
		expect(
			instance.suggestedPostRefunded ?? instance.raw?.suggested_post_refunded,
		).toEqual({} as any);
		expect(
			instance.videoChatScheduled ?? instance.raw?.video_chat_scheduled,
		).toEqual({} as any);
		expect(
			instance.videoChatStarted ?? instance.raw?.video_chat_started,
		).toEqual({} as any);
		expect(instance.videoChatEnded ?? instance.raw?.video_chat_ended).toEqual(
			{} as any,
		);
		expect(
			instance.videoChatParticipantsInvited ??
				instance.raw?.video_chat_participants_invited,
		).toEqual({} as any);
		expect(instance.webAppData ?? instance.raw?.web_app_data).toEqual(
			{} as any,
		);
