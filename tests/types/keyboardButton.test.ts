...
describe("KeyboardButton", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.requestUsers ?? instance.raw?.request_users).toEqual(
			{} as any,
		);
		expect(instance.requestChat ?? instance.raw?.request_chat).toEqual(
			{} as any,
		);
		expect(
			instance.requestManagedBot ?? instance.raw?.request_managed_bot,
		).toEqual({} as any);
		expect(instance.requestContact ?? instance.raw?.request_contact).toEqual(
			true,
		);
		expect(instance.requestLocation ?? instance.raw?.request_location).toEqual(
			true,
		);
		expect(instance.requestPoll ?? instance.raw?.request_poll).toEqual(
			{} as any,
		);
