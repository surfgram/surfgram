...
describe("ChatMemberUpdated", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.oldChatMember ?? instance.raw?.old_chat_member).toEqual(
			{} as any,
		);
		expect(instance.newChatMember ?? instance.raw?.new_chat_member).toEqual(
			{} as any,
		);
		expect(instance.inviteLink ?? instance.raw?.invite_link).toEqual({} as any);
		expect(instance.viaJoinRequest ?? instance.raw?.via_join_request).toEqual(
			true,
		);
		expect(
			instance.viaChatFolderInviteLink ??
				instance.raw?.via_chat_folder_invite_link,
		).toEqual(true);
	});
});
