...
describe("Poll", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.allowsRevoting ?? instance.raw?.allows_revoting).toEqual(
			true,
		);
		expect(instance.membersOnly ?? instance.raw?.members_only).toEqual(true);
		expect(instance.countryCodes ?? instance.raw?.country_codes).toEqual([
			"example text",
		]);
		expect(
			instance.correctOptionIds ?? instance.raw?.correct_option_ids,
		).toEqual([123]);
		...
		expect(
			instance.explanationMedia ?? instance.raw?.explanation_media,
		).toEqual({} as any);
		...
		expect(
			instance.descriptionEntities ?? instance.raw?.description_entities,
		).toEqual([{} as any]);
