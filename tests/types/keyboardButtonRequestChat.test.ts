...
describe("KeyboardButtonRequestChat", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(instance.chatIsChannel ?? instance.raw?.chat_is_channel).toEqual(
			true,
		);
		expect(instance.chatIsForum ?? instance.raw?.chat_is_forum).toEqual(true);
		expect(instance.chatHasUsername ?? instance.raw?.chat_has_username).toEqual(
			true,
		);
		expect(instance.chatIsCreated ?? instance.raw?.chat_is_created).toEqual(
			true,
		);
		expect(
			instance.userAdministratorRights ??
				instance.raw?.user_administrator_rights,
		).toEqual({} as any);
		expect(
			instance.botAdministratorRights ?? instance.raw?.bot_administrator_rights,
		).toEqual({} as any);
		...
		expect(instance.requestUsername ?? instance.raw?.request_username).toEqual(
			true,
		);
