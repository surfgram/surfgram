import { TransactionPartnerUser } from "../../src/api";
import { Bot } from "../../src/core/bot";

describe("TransactionPartnerUser", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		const instance = new TransactionPartnerUser(raw, mockBot);

		expect(instance.raw).toEqual(raw);
		expect(instance.bot).toBe(mockBot);

		expect(instance.type ?? instance.raw?.type).toEqual("example text");
		expect(instance.transactionType ?? instance.raw?.transaction_type).toEqual(
			"example text",
		);
		...
		expect(
			instance.premiumSubscriptionDuration ??
				instance.raw?.premium_subscription_duration,
		).toEqual(123);
	});
});
