...
describe("ChecklistTasksDone", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(
			instance.checklistMessage ?? instance.raw?.checklist_message,
		).toEqual({} as any);
		expect(
			instance.markedAsDoneTaskIds ?? instance.raw?.marked_as_done_task_ids,
		).toEqual([123]);
		expect(
			instance.markedAsNotDoneTaskIds ??
				instance.raw?.marked_as_not_done_task_ids,
		).toEqual([123]);
	});
});
