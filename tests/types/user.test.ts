...
describe("User", () => {
	...
	it("should correctly instantiate from raw data", () => {
		...
		expect(
			instance.supportsGuestQueries ?? instance.raw?.supports_guest_queries,
		).toEqual(true);
		expect(
		...
		expect(
			instance.allowsUsersToCreateTopics ??
				instance.raw?.allows_users_to_create_topics,
		).toEqual(true);
		expect(instance.canManageBots ?? instance.raw?.can_manage_bots).toEqual(
			true,
		);
	});
});
