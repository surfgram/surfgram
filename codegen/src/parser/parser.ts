...
export class Parser {
  ...
  parseTypesAndMethods(): {
    ...
    const h4Regex = /<h4[^>]*>.*?<a\s+[^>]*name="([^"]+)"[^>]*>.*?<\/a>([^<]+)<\/h4>/g;
    const matches: Array<{ id: string; title: string }> = [];
    let match;
    while ((match = h4Regex.exec(this.html))) {
      const id = match[1];
      if (id.includes('changes') || id.includes('release') || id.includes('changelog')) continue;
        ...
        for (const row of rows) {
          ...
          if (tdMatches && tdMatches.length >= 1) {
            ...
            if (typeName && typeName !== 'Field' && /^[A-Z]/.test(typeName)) {
              ...
              if (Validator.isValidTypeName(cleanName)) realTypeNames.add(cleanName);
            }
          }
        }
      }
    ...
    for (const { id, title } of matches) {
      ...
      if (
        (realTypeNames.has(cleanTitle) || Validator.isValidTypeName(cleanTitle)) &&
    ...
    for (const { id, title } of matches) {
      if (/^[a-z]/.test(title) && Validator.isValidMethodName(title)) {
        ...
        const tableMatch = methodSection.match(/<table class="table">([\s\S]*?)<\/table>/);
        if (tableMatch) {
          const rows = tableMatch[1].match(/<tr>([\s\S]*?)<\/tr>/g) || [];
          for (const row of rows) {
            const parsed = this.parseTableRow4Columns(row) || this.parseTableRow3Columns(row);
            if (parsed) {
              parameters.push({
        ...
        if (returnsMatch) {
          ...
          const normalized = TypeNormalizer.normalizeTelegramType(returnText, allTypeNames);
          if (normalized.types.length > 0) returnType = normalized.types.join(' | ');
        }
    ...
    for (const [originalId, type] of Object.entries(types)) {
      ...
      const tableMatch = section.match(/<table class="table">([\s\S]*?)<\/table>/);
      if (tableMatch) {
        const rows = tableMatch[1].match(/<tr>([\s\S]*?)<\/tr>/g) || [];
        for (const row of rows) {
          const parsed = this.parseTableRow4Columns(row) || this.parseTableRow3Columns(row);
          if (parsed) {
            const normalized = TypeNormalizer.normalizeTelegramType(parsed.type, allTypeNames);
            type.fields!.push({
              name: CaseConverter.normalizeFieldName(parsed.name),
    ...
    for (const method of Object.values(methods)) {
      for (const param of method.parameters) {
        const normalized = TypeNormalizer.normalizeTelegramType(param.originalType, allTypeNames);
        param.types = normalized.types;
        normalized.types.forEach((t) => {
          const c = t.replace('[]', '');
          if (c && !['string', 'number', 'boolean', 'any'].includes(c.toLowerCase()))
            allUsedTypes.add(c);
        });
      }
      const rt = method.returnType.replace('[]', '');
      if (rt && !['string', 'number', 'boolean', 'any'].includes(rt.toLowerCase()))
        allUsedTypes.add(rt);
    }
    for (const usedType of allUsedTypes) {
      if (usedType === 'any' || !Validator.isValidTypeName(usedType)) continue;
      if (!Object.values(types).some((t) => t.name.toLowerCase() === usedType.toLowerCase())) {
  ...
  private parseTableRow4Columns(rowHtml: string): {
    ...
    let description = tdMatches[3]
      ...
      .replace(/\s+/g, ' ')
      .trim();
    if (!name || name === 'Field' || name === 'Parameter' || name === 'Parameters') return null;
  ...
  private parseTableRow3Columns(rowHtml: string): {
    ...
    let description = tdMatches[2]
      ...
      .replace(/&amp;/g, '&');
    const required =
      !type.toLowerCase().includes('optional') && !description.toLowerCase().includes('optional');
