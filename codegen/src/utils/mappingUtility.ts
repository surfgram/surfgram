...
export class MappingUtility {
  ...
  static findContextualMappings(type: Type, method: ApiMethod): ContextualMapping[] {
    ...
    const idParameters = method.parameters.filter((param) => {
      ...
      return this.isIdField(paramNameLower) && !this.isNonObjectIdType(paramNameLower);
    });
    ...
    for (const param of idParameters) {
      ...
      if (!mapping.path) {
        mapping = this.findTypeBasedMapping(typeFields, paramNameLower, typeNameLower);
      }
  ...
  private static addIdMappingsForObject(
    ...
    for (const suffix of this.ID_SUFFIXES) {
      const camelId = fieldNameLower.endsWith('id') ? fieldNameLower : `${fieldNameLower}${suffix}`;
  ...
  private static findIndirectMapping(
    ...
    for (const suffix of this.ID_SUFFIXES) {
      if (paramNameLower.endsWith(suffix)) {
        ...
        const cleanBaseName = baseName.endsWith('_') ? baseName.slice(0, -1) : baseName;
        ...
        if (cleanBaseName) {
          const possibleField = this.findSimilarField(typeFields, cleanBaseName);
          if (possibleField) {
  ...
  private static findTypeBasedMapping(
    ...
    for (const field of typeFields) {
      if (this.isObjectType(field)) {
        ...
        if (fieldType) {
          const typeWords = fieldType.split(/(?=[A-Z])/).map((w) => w.toLowerCase());
          const hasMatch = typeWords.some((word) =>
            paramWords.some((paramWord) => paramWord.includes(word) || word.includes(paramWord))
          );
    ...
    const paramWords = paramNameLower.split(/[_\s]+/);
    const typeWords = typeNameLower.split(/(?=[A-Z])/).map((w) => w.toLowerCase());
    ...
    if (paramNameLower.includes('id') || paramNameLower.includes('Id')) {
      ...
      if (objectName) {
        ...
        const matchingFields = typeFields.filter(
          (field) => this.isObjectType(field) && field.name.toLowerCase().includes(objectName)
        );
  ...
  private static findFieldByName(fields: Field[], name: string): Field | undefined {
    return fields.find(
      (f) => f.name.toLowerCase() === name || CaseConverter.camelToSnake(f.name) === name
    );
  }

  private static findSimilarField(fields: Field[], searchName: string): Field | undefined {
