	...
	static isSemanticallyCompatible(method: ApiMethod, type: Type): boolean {
		...
		for (const param of method.parameters) {
			for (const typeName of param.types) {
				if (typeName.replace("[]", "").toLowerCase() === typeNameLower)
					return true;
			}
		}
		if (
			methodNameLower.includes(typeNameLower) ||
			typeNameLower.includes(methodNameLower)
		)
			return true;
		...
		for (const pattern of semanticPatterns) {
			if (
				pattern.method.test(methodNameLower) &&
				pattern.type.test(typeNameLower)
			)
				return true;
		}
		return false;
