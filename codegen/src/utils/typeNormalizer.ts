	...
	static normalizeTelegramType(
		...
		for (let variant of variants) {
			...
			if (lowerVariant === "true" || lowerVariant === "false") {
				...
				["integer", "int", "float", "float number"].includes(lowerVariant)
			) {
				let typeName = "number";
				for (let i = 0; i < arrayLevel; i++) {
					typeName += "[]";
			...
			const matchingType = TypeNormalizer.findMatchingType(
				variant,
				allTypeNames,
			);
			...
			if (
				/^[A-Z][a-z]+([A-Z][a-z]+)*$/.test(variant) &&
				Validator.isValidTypeName(variant)
			) {
