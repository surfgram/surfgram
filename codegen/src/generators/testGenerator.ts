...
export class TestGenerator extends BaseGenerator {
  generate() {
    ...
    typeTests.forEach((t) => this.writeFile(resolve(testsTypesOut, t.file), t.content));
      ...
      .map((method) => {
        ...
        return {
          ...
          content: this.renderTemplate('test_method.ts.njk', {
            ...
            argsString: sortedParams.map((p) => DocUtility.getExampleValue(p.types[0])).join(', '),
          }),
        };
      });
    methodTests.forEach((t) => this.writeFile(resolve(testsMethodsOut, t.file), t.content));
  }
}
