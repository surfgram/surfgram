...
export class DocGenerator extends BaseGenerator {
  generate() {
    ...
    for (const type of Object.values(this.types)) {
      if (Validator.isValidTypeName(type.name)) {
        this.writeFile(resolve(typesDir, `${type.name}.md`), this.renderTypeDoc(type));
      }
    }
    ...
    for (const method of allMethods) {
      this.writeFile(resolve(methodsDir, `${method.name}.md`), this.renderMethodDoc(method));
    }

    this.writeFile(resolve(fluentDir, 'quick-start.md'), this.renderFluentQuickStart());
    this.writeFile(
    ...
    const typesWithFluent = Object.values(this.types).filter(
      (t) =>
        ...
        Object.values(this.methods).some((m) => Validator.isSemanticallyCompatible(m, t))
    );
      ...
      if (method.parameters.length === 0) {
        ...
        if (requiredParams.length > 0) {
          directCall += requiredParams
            .slice(0, 3)
            .map((p) => `  ${p.name}: ${DocUtility.getExampleValue(p.types[0])},`)
            .join('\n');
        }
        directCall += '\n});';
      }
  ...
  private prepareMethodForType(type: Type, method: ApiMethod) {
    ...
    const autoMappings = mappings
      ...
      .map((m) => ({
        ...m,
        description: method.parameters.find((p) => p.name === m.parameterName)?.description || '',
      }));
    ...
    if (useParamsObject) {
      ...
      } else if (nonAutoParams.some((p) => p.types.includes('string') || p.name.includes('text'))) {
        const p = nonAutoParams.find((p) => p.types.includes('string') || p.name.includes('text'));
