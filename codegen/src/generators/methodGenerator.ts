...
export class MethodGenerator extends BaseGenerator {
  generate() {
    ...
    for (const method of validMethods) {
      ...
      const importMap = new Map<string, { name: string; file: string; isInterface: boolean }>();
      if (useParamsObject) {
        importMap.set(interfaceName, {
          name: interfaceName,
          file: CaseConverter.toCamelCaseFileName(interfaceName).replace('.ts', ''),
          isInterface: true,
        });
      }
      for (const param of method.parameters) {
        for (const tName of param.types) {
          ...
          if (
            clean === 'any' ||
            BASE_TYPE_MAP[clean] ||
            ['string', 'number', 'boolean', 'true', 'false'].includes(clean.toLowerCase())
          )
            continue;
      ...
      if (
        ...
        !['string', 'number', 'boolean', 'any'].includes(retTypeClean.toLowerCase()) &&
        this.allTypeNames.includes(retTypeClean) &&
        !importMap.has(retTypeClean)
      ) {
        importMap.set(retTypeClean, {
          name: retTypeClean,
          file: CaseConverter.toCamelCaseFileName(retTypeClean).replace('.ts', ''),
          isInterface: false,
        });
      }
      const imports = Array.from(importMap.values());
      const sortedParams = SortUtility.sortParameters(method.parameters).map((p) => ({
        ...p,
